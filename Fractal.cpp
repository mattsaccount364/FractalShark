...
{ // Create the control-key-down/mouse-movement-monitoring thread.
    ...
    //SetPerturbationAlg(PerturbationAlg::MTPeriodicity2Perturb);
...
void Fractal::View(size_t view)
{
    ...
    case 13:
        minX = HighPrecision{ "-1." };
        minY = HighPrecision{ "-0." };
        maxX = HighPrecision{ "-1." };
        maxY = HighPrecision{ "-0." };
        SetNumIterations(2880000);
        break;
...
void Fractal::DrawPerturbationResults(bool LeaveScreen) {
    ...
    {
        if (IsPerturbationResultUsefulHere<T, false>(i)) {
...
void Fractal::CalcCpuPerturbationFractal(bool MemoryOnly) {
    auto* results = GetAndCreateUsefulPerturbationResults<double, double>();
...
void Fractal::CalcCpuPerturbationFractalBLA(bool MemoryOnly) {
    auto* results = GetAndCreateUsefulPerturbationResults<T, SubType>();
...
bool Fractal::RequiresBadCalc() const {
    switch (GetRenderAlgorithm()) {
    case RenderAlgorithm::GpuHDRx32PerturbedScaled:
    case RenderAlgorithm::Gpu1x32PerturbedScaled:
    case RenderAlgorithm::Gpu1x32PerturbedScaledBLA:
...
template<class T>
std::vector<std::unique_ptr<PerturbationResults<T>>>& Fractal::GetPerturbationResults() {
...
void Fractal::AddPerturbationReferencePoint() {
    if (m_PerturbationGuessCalcX == 0 && m_PerturbationGuessCalcY == 0) {
        m_PerturbationGuessCalcX = (m_MaxX + m_MinX) / HighPrecision(2);
        m_PerturbationGuessCalcY = (m_MaxY + m_MinY) / HighPrecision(2);
    }

    if (RequiresBadCalc()) {
        if (m_PerturbationAlg == PerturbationAlg::ST) {
            AddPerturbationReferencePointST<T, SubType, false, BenchmarkMode, true>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MT) {
            AddPerturbationReferencePointMT2<T, SubType, false, BenchmarkMode, true>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::STPeriodicity) {
            AddPerturbationReferencePointST<T, SubType, true, BenchmarkMode, true>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MTPeriodicity2 || m_PerturbationAlg == PerturbationAlg::MTPeriodicity2Perturb) {
            AddPerturbationReferencePointMT2<T, SubType, true, BenchmarkMode, true>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MTPeriodicity5) {
            AddPerturbationReferencePointMT5<T, SubType, true, BenchmarkMode, true>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
    } else {
        if (m_PerturbationAlg == PerturbationAlg::ST) {
            AddPerturbationReferencePointST<T, SubType, false, BenchmarkMode, false>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MT) {
            AddPerturbationReferencePointMT2<T, SubType, false, BenchmarkMode, false>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::STPeriodicity) {
            AddPerturbationReferencePointST<T, SubType, true, BenchmarkMode, false>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MTPeriodicity2 || m_PerturbationAlg == PerturbationAlg::MTPeriodicity2Perturb) {
            AddPerturbationReferencePointMT2<T, SubType, true, BenchmarkMode, false>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
        else if (m_PerturbationAlg == PerturbationAlg::MTPeriodicity5) {
            AddPerturbationReferencePointMT5<T, SubType, true, BenchmarkMode, false>(
                m_PerturbationGuessCalcX,
                m_PerturbationGuessCalcY);
        }
    }
...
void Fractal::AddPerturbationReferencePointST(HighPrecision initX, HighPrecision initY) {
    ...
    results->ReuseX.reserve(m_NumIterations);
    results->ReuseY.reserve(m_NumIterations);
...
template<class T, class SubType, bool Periodicity, bool BenchmarkMode, bool CalcBad>
bool Fractal::AddPerturbationReferencePointSTReuse(HighPrecision initX, HighPrecision initY) {
    auto& PerturbationResultsArray = GetPerturbationResults<T>();
    PerturbationResultsArray.push_back(std::make_unique<PerturbationResults<T>>());
...
#ifndef FULL_PREC_TEST_ONLY
    auto NewPrec = GetPrecision(m_MinX, m_MinY, m_MaxX, m_MaxY);
    ...
    //results->x.push_back(T(cx));
    //results->y.push_back(T(cy));
    ...
    if constexpr (CalcBad) {
        results->bad.reserve(m_NumIterations);
        results->bad.resize(m_NumIterations); // TODO
        assert(false);
        // Not implemented yet
    }
...
//    InitReused(results);
    ...
    for (i = 0; i < m_NumIterations; i++)
    {
        ...
        reuseX = existingResults->ReuseX[RefIteration];
#ifndef FULL_PREC_TEST_ONLY
        reuseX.precision(precNum);
#endif

        reuseY = existingResults->ReuseY[RefIteration];
#ifndef FULL_PREC_TEST_ONLY
        reuseY.precision(precNum);
#endif
...
bool Fractal::IsReferencePerturbationEnabled() const {
...
template<class T, class SubType>
PerturbationResults<T>* Fractal::GetAndCreateUsefulPerturbationResults() {
    bool added = false;
    if (IsReferencePerturbationEnabled()) {
        ...
        if (results == nullptr) {
            added = AddPerturbationReferencePointSTReuse<T, SubType, true, false, false>(m_PerturbationGuessCalcX, m_PerturbationGuessCalcY);
        }
    }
...
template<class T>
std::vector<std::unique_ptr<PerturbationResults<T>>>* Fractal::GetPerturbationArray() {
