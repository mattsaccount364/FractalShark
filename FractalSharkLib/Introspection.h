...
template <PerturbExtras PExtras> struct TestPExtras {
    static constexpr bool value = (PExtras != PerturbExtras::MaxCompression);
};

template <typename IterType, typename Float, PerturbExtras PExtras> struct PerturbTypeParams {
    using IterType_ = IterType;
    using Float_ = Float;
    static constexpr auto PExtras_ = PExtras;
};

template <template <typename, typename, PerturbExtras> typename PerturbType,
          typename IterType,
          typename Float,
          PerturbExtras PExtras>
constexpr auto Extract(const PerturbType<IterType, Float, PExtras> &)
    -> PerturbTypeParams<IterType, Float, PExtras>;
...
IsDblFlt()
{
    return std::is_same<Extract_Float<PerturbType>, CudaDblflt<MattDblflt>>::value ||
...
IsTDblFlt()
{
    return std::is_same<T, CudaDblflt<MattDblflt>>::value ||
