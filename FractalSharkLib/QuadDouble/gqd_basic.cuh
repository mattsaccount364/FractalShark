...
quick_renorm(double &c0, double &c1, double &c2, double &c3, double &c4)
{
    double t0, t1, t2, t3;
    double s;
...
__host__ __device__ void
renorm(double &c0, double &c1, double &c2, double &c3)
{
...
__host__ __device__ void
renorm(double &c0, double &c1, double &c2, double &c3, double &c4)
{
    double s0, s1, s2 = 0.0, s3 = 0.0;
...
/** additions */
__host__ __device__ void
three_sum(double &a, double &b, double &c)
{
    double t1, t2, t3;
    t1 = two_sum(a, b, t2);
    a = two_sum(c, t1, t3);
...
__host__ __device__ void
three_sum2(double &a, double &b, double &c)
{
    double t1, t2, t3;
    t1 = two_sum(a, b, t2);
    a = two_sum(c, t1, t3);
...
/// qd = qd + double
__host__ __device__ gqd_real
operator+(const gqd_real &a, double b)
{
    double c0, c1, c2, c3;
    double e;
...
/// qd = double + qd
__host__ __device__ gqd_real
operator+(double a, const gqd_real &b)
{
    return (b + a);
}
...
/// qd = qd + qd
__host__ __device__ gqd_real
sloppy_add(const gqd_real &a, const gqd_real &b)
{
    double s0, s1, s2, s3;
    double t0, t1, t2, t3;
...
__host__ __device__ gqd_real
operator+(const gqd_real &a, const gqd_real &b)
{
...
/** multiplications */
__host__ __device__ gqd_real
mul_pwr2(const gqd_real &a, double b)
{
...
__device__ gqd_real
sloppy_mul(const gqd_real &a, const gqd_real &b)
{
    ...
    s1 = s1 + (__dmul_rn(a.x, b.w) + __dmul_rn(a.y, b.z) + __dmul_rn(a.z, b.y) + __dmul_rn(a.w, b.x) +
               q0 + q3 + q4 + q5);
...
/** divisions */
__device__ gqd_real
sloppy_div(const gqd_real &a, const gqd_real &b)
{
    double q0, q1, q2, q3;
...
__host__ __device__ bool
is_zero(const gqd_real &x)
{
    return (x.x == 0.0);
}

__host__ __device__ bool
is_one(const gqd_real &x)
{
    return (x.x == 1.0 && x.y == 0.0 && x.z == 0.0 && x.w == 0.0);
}

__host__ __device__ bool
is_positive(const gqd_real &x)
{
    return (x.x > 0.0);
}

__host__ __device__ bool
is_negative(const gqd_real &x)
{
    return (x.x < 0.0);
}

__device__ gqd_real
nint(const gqd_real &a)
{
    double x0, x1, x2, x3;
