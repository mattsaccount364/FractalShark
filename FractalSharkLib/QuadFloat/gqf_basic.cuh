...
quick_renorm(float &c0, float &c1, float &c2, float &c3, float &c4)
{
    float t0, t1, t2, t3;
    float s;
...
__host__ __device__ void
renorm(float &c0, float &c1, float &c2, float &c3)
{
...
__host__ __device__ void
renorm(float &c0, float &c1, float &c2, float &c3, float &c4)
{
    float s0, s1, s2 = 0.0f, s3 = 0.0f;
...
/** additions */
__host__ __device__ void
three_sum(float &a, float &b, float &c)
{
    float t1, t2, t3;
    t1 = two_sum(a, b, t2);
    a = two_sum(c, t1, t3);
...
__host__ __device__ void
three_sum2(float &a, float &b, float &c)
{
    float t1, t2, t3;
    t1 = two_sum(a, b, t2);
    a = two_sum(c, t1, t3);
...
/// qd = qd + float
__host__ __device__ gqf_real
operator+(const gqf_real &a, float b)
{
    float c0, c1, c2, c3;
    float e;
...
/// qd = float + qd
__host__ __device__ gqf_real
operator+(float a, const gqf_real &b)
{
    return (b + a);
}
...
/// qd = qd + qd
__host__ __device__ gqf_real
sloppy_add(const gqf_real &a, const gqf_real &b)
{
    float s0, s1, s2, s3;
    float t0, t1, t2, t3;
...
__host__ __device__ gqf_real
operator+(const gqf_real &a, const gqf_real &b)
{
...
/** multiplications */
__host__ __device__ gqf_real
mul_pwr2(const gqf_real &a, float b)
{
...
__device__ gqf_real
sloppy_mul(const gqf_real &a, const gqf_real &b)
{
    ...
    s1 = s1 + (__fmul_rn(a.x, b.w) + __fmul_rn(a.y, b.z) + __fmul_rn(a.z, b.y) + __fmul_rn(a.w, b.x) +
               q0 + q3 + q4 + q5);
...
/** divisions */
__device__ gqf_real
sloppy_div(const gqf_real &a, const gqf_real &b)
{
    float q0, q1, q2, q3;
...
__host__ __device__ bool
is_zero(const gqf_real &x)
{
    return (x.x == 0.0f);
}

__host__ __device__ bool
is_one(const gqf_real &x)
{
    return (x.x == 1.0f && x.y == 0.0f && x.z == 0.0f && x.w == 0.0f);
}

__host__ __device__ bool
is_positive(const gqf_real &x)
{
    return (x.x > 0.0f);
}

__host__ __device__ bool
is_negative(const gqf_real &x)
{
    return (x.x < 0.0f);
}

__device__ gqf_real
nint(const gqf_real &a)
{
    float x0, x1, x2, x3;
