...
std::unique_ptr<Color16[]> RenderThreadPool::AcquireFrameBuffer(size_t totalPixels) {
    ...
    return std::make_unique<Color16[]>(totalPixels);
}
...
void RenderThreadPool::WorkerLoop(size_t workerIndex) {
    ...
    // Each worker has its own mutex/CV for GPU compute-done notification
    ...
    while (!m_ShutdownFlag.load()) {
        ...
        // Register this worker's CV with the renderer for compute-done signals
        auto &renderer = fractal->GetRenderer(rendererIdx);
        renderer.SetComputeDoneNotification(&workerMutex, &workerCV);

        // Acquire our own ItersMemoryContainer from the pool.
        // This avoids all workers sharing m_CurIters.
        ItersMemoryContainer workerIters = fractal->AcquireItersMemory();

        // Swap-and-restore: briefly hold the lock, swap our container into
        // m_CurIters so CalcFractal uses it, then swap back.
        // Also restore dirty flags from the snapshot so CalcFractal sees
        // the state that was live at enqueue time (otherwise a prior worker's
        // ChangedMakeClean() would make this worker skip computation).
        {
            std::lock_guard lk(m_CalcFractalMutex);
            std::swap(fractal->m_CurIters, workerIters);

            // Save and restore coordinates + dirty flags around CalcFractal
            // so it uses the state snapshotted at enqueue time.
            auto savedPtz = fractal->m_Ptz;
            bool savedWindow = fractal->m_ChangedWindow;
            bool savedScrn = fractal->m_ChangedScrn;
            bool savedIters = fractal->m_ChangedIterations;

            fractal->m_Ptz = item.Ptz;
            ...
            fractal->CalcFractal(rendererIdx, false);

            fractal->m_Ptz = savedPtz;
            fractal->m_ChangedWindow = savedWindow;
            fractal->m_ChangedScrn = savedScrn;
            fractal->m_ChangedIterations = savedIters;

            std::swap(fractal->m_CurIters, workerIters);
        }
        // Now workerIters has our compute results; m_CurIters is restored.

        // Progressive rendering loop: produce frames while GPU computes.
        // No lock needed — we own workerIters and the renderer.
        if (fractal->m_BypassGpu == false) {
            ...
            for (;;) {
                ...
                // GPU still computing — produce progressive frame
                ProduceFrame(item, rendererIdx, workerIters, false);
            }
        }

        // Produce final frame with complete data
        if (!m_ShutdownFlag.load()) {
...
void RenderThreadPool::GlConsumerLoop() {
    ...
    while (!m_ShutdownFlag.load()) {
        if (!m_FrameQueue.WaitForFrame(nextExpectedSeqNum)) {
            break; // Shutdown
        }

        RenderFrame frame;
        while (m_FrameQueue.TryPopNextInOrder(nextExpectedSeqNum, frame)) {
            // Render the frame via GL
...
void RenderThreadPool::ProduceFrame(
    ...
    auto colorData = AcquireFrameBuffer(totalPixels);
