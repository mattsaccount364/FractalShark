...
template <class T> class HDRFloatComplex;

template <class T> class FloatComplex;
...
enum class HDROrder { Left, Right };
...
template <class T, class TExp = int32_t> class GenericHdrBase {
public:
    static CUDA_CRAP constexpr TExp
    MIN_BIG_EXPONENT()
    {
        if constexpr (std::is_same<TExp, int32_t>::value || std::is_same<TExp, float>::value) {
            return INT32_MIN >> 3;
        } else {
            return INT16_MIN >> 3;
        }
...
template <class T, class TExp = int32_t> class LMembers : public GenericHdrBase<T, TExp> {
public:
...
template <class T, class TExp = int32_t> class RMembers : public GenericHdrBase<T, TExp> {
public:
    using GenericHdrBase = GenericHdrBase<T, TExp>;
    CUDA_CRAP
    RMembers()
...
class HDRFloat
    : public std::conditional<Order == HDROrder::Left, LMembers<T, TExp>, RMembers<T, TExp>>::type {
    // #else
...
public:
    ...
    static CUDA_CRAP constexpr TExp
    MIN_SMALL_EXPONENT_FLOAT()
    {
        return -127;
    }

    static CUDA_CRAP constexpr TExp
    MIN_SMALL_EXPONENT_DOUBLE()
    {
        return -1023;
    }

    static CUDA_CRAP constexpr int32_t
    MIN_SMALL_EXPONENT_INT_FLOAT()
    {
        return -127;
    }
...
public:
    ...
    ToString() const
    {
        constexpr bool isDblFlt =
            std::is_same<T, CudaDblflt<dblflt>>::value || std::is_same<T, CudaDblflt<MattDblflt>>::value;
    ...
    template <HDROrder OtherOrder> CUDA_CRAP constexpr HDRFloat(HDRFloat<T, OtherOrder> &&other)
    {
        Base::mantissa = other.mantissa;
        Base::exp = other.exp;
    }
    ...
    CUDA_CRAP constexpr HDRFloat &
    operator=(const HDRFloat<T, OtherOrder> &other)
    {
        Base::mantissa = other.mantissa;
        Base::exp = other.exp;
        return *this;
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr HDRFloat &
    operator=(HDRFloat<T, OtherOrder> &&other)
    {
        Base::mantissa = other.mantissa;
        Base::exp = other.exp;
        return *this;
    ...
    template <class U,
              typename = std::enable_if_t<
                  ...
                  std::is_same<U, int>::value || std::is_same<U, CudaDblflt<dblflt>>::value>>
    CUDA_CRAP explicit HDRFloat(const U number)
    { // TODO add constexpr once that compiles
        if (number == U{}) {
    ...
    explicit HDRFloat(const HighPrecisionT<HPDestructor::True> &number) : HDRFloat{*number.backendRaw()}
    {
    }

    void
    GetHighPrecision(HighPrecisionT<HPDestructor::True> &number) const
    {
        mpf_t temp;
        mpf_init2(temp, 64);
    ...
    Reduce(TExp *DestExp = nullptr) &
    {
        if constexpr (std::is_same<T, double>::value) {
            ...
            const auto f_exp_x = static_cast<TExp>(((bits_x & 0x7F80'0000UL) >> 23UL));
    ...
    getMultiplier(TExp scaleFactor)
    {
    ...
    CUDA_CRAP constexpr T
    toDoubleSub(TExp exponent) const
    {
        return Base::mantissa * getMultiplier(Base::exp - exponent);
    }

    CUDA_CRAP explicit constexpr
    operator T() const
    {
        return toDouble();
    }
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP bool
    operator==(const HDRFloat<T, OtherOrder> &other) const
    {
        if (Base::exp == other.exp && Base::mantissa == other.mantissa) {
            return true;
        }
    ...
    CUDA_CRAP constexpr TExp
    getExp() const
    {
        return Base::exp;
    }

    CUDA_CRAP constexpr void
    setExp(TExp localexp)
    {
        Base::exp = localexp;
    }
    ...
    divide_mutable(HDRFloat<T, OtherOrder, typename OtherTExp> factor)
    {
        ...
        Base::exp = local_exp < GenericHdrBase::MIN_BIG_EXPONENT() ? GenericHdrBase::MIN_BIG_EXPONENT()
                                                                   : local_exp;
    ...
    template <HDROrder OtherOrder, typename OtherTExp>
    friend CUDA_CRAP constexpr HDRFloat
    operator/(HDRFloat lhs, // passing lhs by value helps optimize chained a+b+c
              const HDRFloat<T, OtherOrder, OtherTExp>
    ...
    friend CUDA_CRAP constexpr HDRFloat
    operator/(
        HDRFloat<T, OtherOrder, OtherTExp> lhs, // passing lhs by value helps optimize chained a+b+c
    ...
    template <HDROrder OtherOrder, typename OtherTExp>
    CUDA_CRAP constexpr HDRFloat &
    operator/=(const HDRFloat<T, OtherOrder, OtherTExp> &other)
    {
        return divide_mutable(other);
    }

    template <bool plus>
    static CUDA_CRAP HDRFloat
    custom_perturb1(const HDRFloat &DeltaSubNXOrig,
    ...
    custom_perturb2(HDRFloat &DeltaSubNX,
                    HDRFloat &DeltaSubNY,
                    ...
                    const HDRFloat &DeltaSub0Y)
    {
        ...
        // DeltaSubNX = HDRFloat(ZeroOrOne * finalMaxexpF, ZeroOrOne * __fmaf_rn(DeltaSub0X.mantissa,
        // mul2F, mantissaSum1F)) +
        //              HDRFloat((1 - ZeroOrOne) * finalMaxexpF, (1 - ZeroOrOne) *
        //              __fmaf_rn(mantissaSum1F, mul2F, DeltaSub0X.mantissa));
        if (expDiff2F >= 0) {
    ...
    static CUDA_CRAP void
    custom_perturb3(HDRFloat &DeltaSubNX,
                    HDRFloat &DeltaSubNY,
                    const HDRFloat &DeltaSubNXOrig,
                    const HDRFloat &tempSum1,
                    const HDRFloat &DeltaSubNYOrig,
                    ...
                    const HDRFloat &DeltaSub0Y)
    {
        DeltaSubNX = DeltaSubNXOrig * tempSum1 - DeltaSubNYOrig * tempSum2 + DeltaSub0X;
        HdrReduce(DeltaSubNX);

        DeltaSubNY = DeltaSubNXOrig * tempSum2 + DeltaSubNYOrig * tempSum1 + DeltaSub0Y;
        HdrReduce(DeltaSubNY);
    ...
    template <HDROrder OtherOrder, typename OtherTExp>
    CUDA_CRAP constexpr HDRFloat &
    multiply_mutable(const HDRFloat<T, OtherOrder, OtherTExp> &factor)
    {
        T local_mantissa = Base::mantissa * factor.mantissa;
        TExp local_exp = Base::exp + factor.exp;
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr HDRFloat
    add(HDRFloat<T, OtherOrder> value) const
    {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr HDRFloat &
    add_mutable(HDRFloat<T, OtherOrder> value)
    {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr HDRFloat
    subtract(HDRFloat<T, OtherOrder> value) const
    {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr HDRFloat &
    subtract_mutable(HDRFloat<T, OtherOrder> value)
    {
    ...
    CUDA_CRAP constexpr HDRFloat
    subtract(T value) const
    {
        return subtract(HDRFloat(value));
    }

    CUDA_CRAP constexpr HDRFloat &
    subtract_mutable(T value)
    {
    ...
    template <HDROrder OtherOrder>
    friend CUDA_CRAP constexpr HDRFloat
    operator-(HDRFloat<T, OtherOrder> lhs)
    {
        return lhs.negate();
    }

    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr int
    compareToBothPositiveReduced(HDRFloat<T, OtherOrder> compareTo) const
    {
    ...
    compareToBothPositiveReducedTemplate() const
    {
        if (Base::exp > 1) {
            return 1;
        } else if (Base::exp < 1) {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr int
    compareToBothPositive(HDRFloat<T, OtherOrder> compareTo) const
    {
        if (Base::exp > compareTo.exp) {
            return 1;
        } else if (Base::exp < compareTo.exp) {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr int
    compareTo(HDRFloat<T, OtherOrder> compareTo) const
    {
    ...
    template <HDROrder OtherOrder>
    CUDA_CRAP constexpr int
    compareToReduced(HDRFloat<T, OtherOrder> compareToReduced) const
    {
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    HDRMax(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    maxBothPositive(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    maxBothPositiveReduced(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    minBothPositive(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
        return a.compareToBothPositive(b) < 0 ? a : b;
    }
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    minBothPositiveReduced(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
        return a.compareToBothPositiveReduced(b) < 0 ? a : b;
    }
    ...
    template <HDROrder OtherOrder>
    static CUDA_CRAP constexpr HDRFloat
    HDRMin(HDRFloat a, HDRFloat<T, OtherOrder> b)
    {
        return a.compareTo(b) < 0 ? a : b;
    }
};
...
template <class T>
static CUDA_CRAP constexpr T
HdrAbs(const T &incoming)
requires(std::is_same<T, double>::value || std::is_same<T, float>::value ||
         std::is_same<T, CudaDblflt<MattDblflt>>::value || std::is_same<T, HDRFloat<double>>::value ||
         std::is_same<T, HDRFloat<float>>::value || std::is_same<T, HDRFloat<CudaDblflt<dblflt>>>::value)
{
...
template <class T>
static CUDA_CRAP constexpr void
HdrReduce(T &incoming)
{
    ...
    static_assert(
        std::is_same_v<T, double> || std::is_same_v<T, float> || std::is_same_v<T, CudaDblflt<dblflt>> ||
            ...
            std::is_same_v<T, Imagina::HRReal> || std::is_same_v<T, HDRFloatComplex<double>> ||
            std::is_same_v<T, HDRFloatComplex<float>> || std::is_same_v<T, FloatComplex<double>> ||
            std::is_same_v<T, FloatComplex<float>> || HighPrecPossible,
        "No");

    if constexpr (std::is_same<T, HDRFloat<double>>::value || std::is_same<T, HDRFloat<float>>::value ||
...
HdrReduce(T &&incoming)
{
    ...
    static_assert(std::is_same<no_const, double>::value || std::is_same<no_const, float>::value ||
                      ...
                      std::is_same<no_const, HDRFloatComplex<float>>::value || HighPrecPossible,
                  "No");
...
static CUDA_CRAP constexpr T
HdrMaxReduced(const T &one, const U &two)
{
    static_assert(std::is_same<T, double>::value || std::is_same<T, float>::value ||
                      std::is_same<T, HDRFloat<double>>::value ||
                      std::is_same<T, HDRFloat<float>>::value,
...
template <class T, class U>
static CUDA_CRAP constexpr bool
HdrCompareToBothPositiveReducedLT(const T &one, const U &two)
{
    constexpr auto HighPrecPossible = // LOLZ I imagine there's a nicer way to do this here
#ifndef __CUDACC__
        std::is_same<T, HighPrecisionT<HPDestructor::False>>::value ||
    ...
    static_assert(std::is_same<T, double>::value || std::is_same<T, float>::value ||
                      ...
                      std::is_same<T, HDRFloat<double>>::value ||
                      std::is_same<T, HDRFloat<float>>::value ||
                      std::is_same<T, HDRFloat<CudaDblflt<dblflt>>>::value || HighPrecPossible,
                  "No");
...
static CUDA_CRAP constexpr bool
HdrCompareToBothPositiveReducedLT(const T &one)
{
    constexpr auto HighPrecPossible = // LOLZ I imagine there's a nicer way to do this here
#ifndef __CUDACC__
        std::is_same<T, HighPrecisionT<HPDestructor::False>>::value ||
...
template <class T, class U>
static CUDA_CRAP constexpr bool
HdrCompareToBothPositiveReducedLE(const T &one, const U &two)
{
    constexpr auto HighPrecPossible = // LOLZ I imagine there's a nicer way to do this here
#ifndef __CUDACC__
        std::is_same<T, HighPrecisionT<HPDestructor::False>>::value ||
    ...
    static_assert(std::is_same<T, double>::value || std::is_same<T, float>::value ||
                      std::is_same<T, HDRFloat<double>>::value ||
                      std::is_same<T, HDRFloat<float>>::value || HighPrecPossible,
                  "No");
...
template <class T, class U>
static CUDA_CRAP constexpr bool
HdrCompareToBothPositiveReducedGT(const T &one, const U &two)
{
    constexpr auto HighPrecPossible = // LOLZ I imagine there's a nicer way to do this here
#ifndef __CUDACC__
        std::is_same<T, HighPrecisionT<HPDestructor::False>>::value ||
...
template <class T, class U>
static CUDA_CRAP constexpr bool
HdrCompareToBothPositiveReducedGE(const T &one, const U &two)
{
    constexpr auto HighPrecPossible = // LOLZ I imagine there's a nicer way to do this here
#ifndef __CUDACC__
        std::is_same<T, HighPrecisionT<HPDestructor::False>>::value ||
...
static CUDA_CRAP std::string
HdrToString(const T &dat)
{
    if constexpr (std::is_same<T, double>::value || std::is_same<T, float>::value) {
...
static CUDA_CRAP double
HdrFromIntToDbl(const std::string &mantissaStr)
{
...
template <typename T, typename SubType> class DoubleTo2x32Converter {
public:
    ...
    using ConditionalT =
        typename std::conditional<ConditionalResult1,
                                  HDRFloat<double>,
                                  typename std::conditional<ConditionalResult2, double, T>::type>::type;

    using ConditionalSubType = typename std::conditional<
        ConditionalResult1,
        double,
        typename std::conditional<ConditionalResult2, double, SubType>::type>::type;
};
...
HdrLdexp(const HDRFloat<MantT, Order, ExpT> &x, int e)
{
    // x * 2^e  => add to big exponent
    // Preserve "zero representation" (mantissa==0) as-is.
    if (x.getMantissa() == MantT{}) {
        return x;
    }

    auto t = x;
    // NOTE: your ExpT might be int32_t / int64_t / float (!!)
    // For float exponents, adding int is still fine.
    t.setExp((ExpT)(t.getExp() + (ExpT)e));
    // If you want strict normalization guarantees, you can Reduce() here,
    // but it is usually not needed since mantissa unchanged.
    // t.Reduce();
    return t;
}

// --------------------
// HdrLdexp: float/double
// --------------------
template <class T>
CUDA_CRAP inline T
HdrLdexp(T x, int e)
