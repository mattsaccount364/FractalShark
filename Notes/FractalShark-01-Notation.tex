...
$D$          & number of 32-bit \code{HpSharkFloat} limbs
             & compile-time \code{GlobalNum\allowbreak Uint32} in the fused NTT schedule \\
$L$          & number of packed base-$2^b$ coefficients
             & $L=\lceil 32D/b\rceil$ \\
$M$          & active power-of-two NTT length in the fused schedule
             & $M\ge 2L$; Ref2 may enlarge it for shifted support, up to $2^{25}$ \\
$b$          & packed NTT coefficient width in bits
             & $b=16$ for CUDA Ref2 and its host reference model \\
\hline
\end{tabularx}
\end{table}
