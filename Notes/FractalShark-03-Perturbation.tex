...
\[
\code{ZCoeff}_0 = 1,\qquad \code{CCoeff}_0 = 1.
\]
...
stages from coarsest to finest.  In the inner loop (lines 101--120),
...
The AT is constructed from the topmost LA stage's first entry (call it
LA\(_0\) with coefficients \code{ZCoeff}, \code{CCoeff} and the
\emph{next} entry's reference \(z_\mathrm{ref,next}\)).  The
