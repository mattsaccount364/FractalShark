...
\caption[Iteration skipping: perturbation vs.\ BLA vs.\ LA]{%
  ...
  step length varies with the local linearization error---shorter
  near orbit inflection points, longer on smooth stretches.
  \emph{LA~v2} (red, solid) aligns its approximation hierarchy with
  the orbit period: each stage-0 entry skips exactly one
  period~$p$, reducing cost to ${\sim}4$~evaluations.  Higher
  stages (red, dashed) compose stage-0 entries to skip multiples
  of~$p$ in a single step.}
...
This connection is not a coincidence: \code{CCoeff} accumulates the total
...
\code{LAThresholdScale} is an empirically chosen tunable parameter
(default \(2^{-24}\), set by \code{Default\-LA\-Threshold\-Scale\-Exponent} in \code{LAParameters.h}); no formal derivation is known.
The \(\min\) in \eqref{eq:la-threshold} ensures the threshold can only decrease as more iterations are
...
This structure is analogous to BLA composition (\cref{sec:bilinear-approx}):
...
\begin{enumerate}
  \item \textbf{Period detection (stage~0).}  The algorithm scans the
        ...
        \begin{itemize}
          \item \textbf{Method~1 (minimum-magnitude).}  The algorithm
                ...
                This test is the default method.
  ...
  \item \textbf{Fallback for aperiodic orbits.}  If no period is
        ...
        proceeds with that period length.  This fallback ensures LA~v2 provides
  ...
  \item \textbf{Subdivision of large periods.}  If a true period
        ...
        This subdivision keeps each stage's entry count manageable.
\end{enumerate}

The observation underlying this step is that near a mini-Mandelbrot copy, the orbit nearly
...
Prepare check fails at stage~\(s\), the kernel does not
restart from entry~0 of stage~\(s-1\);instead, it resumes at
macro-iteration index \code{NextStageLAIndex}, which points to the
correct position within stage~\(s-1\)'s entry array.  This indirection avoids
...
counter at entry~0 of the current stage.  This behavior mirrors the rebasing
...
\(|w|^2 \ge \code{AT.SqrEscapeRadius}\).  No reference-orbit
lookups are needed---only scalar iteration.  Each AT iteration corresponds
...
The AT can provide a substantial speedup when applicable.  At deep zoom near
...
only the perturbation delta.  Given a single LA step with coefficients
...
\begin{itemize}
  ...
  \item \textbf{Prepare step.}  BLA applies a purely linear map at every
        ...
        iterations.  This design provides better accuracy per LA step.
