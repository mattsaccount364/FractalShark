...
of massively parallel architectures while preserving exact arithmetic
semantics.
...
Crucially, the decoupled look-back design allows blocks to complete their local
...
\emph{scan}) is a fundamental parallel primitive: given an array of
...
The key insight is that \emph{any} associative operator works---including
...
Fundamentally, the NTT implementation in \FractalShark{} does not prioritize
...
decimation-in-time (DIT) formulation.  This requires a
\emph{bit-reversal permutation} as a preprocessing step: each element at
index~$i$ is swapped with the element at index~$\mathrm{bitrev}(i)$,
where $\mathrm{bitrev}$ reverses the $\log_2 N$ least-significant bits of
the index.  The permutation ensures that the iterative butterfly stages
can read and write elements at naturally strided offsets, enabling
coalesced memory access patterns on the GPU.

In \FractalShark{}'s implementation, the bit-reversal is performed
in-place across all active arrays simultaneously
(\code{BitReverseInplace64\_GridStride} with \code{FourWay} or
\code{SevenWay} mode), so that a single grid-strided pass permutes all
input arrays before any butterfly work begins.  Each thread computes
$j = \mathrm{bitrev}(i)$ via the hardware \code{\_\_brev} intrinsic and
swaps elements $i$ and $j$ (with the $i < j$ guard ensuring each pair is
...
the inverse (or vice versa)---can cancel the two permutations entirely,
since the bit-reversed output of the forward transform is already in the
correct input order for a DIF inverse.  This ``self-sorting'' approach
eliminates two global-memory permutation passes per multiply (one forward,
one inverse), each of which involves random-access swaps across the full
$N$-element array.

\FractalShark{} currently uses DIT for both forward and inverse
transforms, incurring an explicit bit-reversal permutation before each.
Adopting a DIT/DIF mixed strategy is a potential optimization that would
remove these permutation passes and their associated memory traffic,
at the cost of a different butterfly indexing pattern in the inverse
transform.
...
performed to produce a canonical high-precision result.  Carry propagation
proceeds in two phases: first, two rounds of simple ripple carry reduce
the wide accumulator values into 32-bit limbs with small residual carries
(0~or~1), bringing the data into the standard form expected by the
parallel prefix scan.  If any residual carries remain, the same
decoupled-look-back parallel prefix scan used for addition
(\cref{sec:hp-add}) resolves them.
