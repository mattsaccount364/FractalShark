...
critical orbitfrom $z_0 = 0$ for exactly $p$~steps, accumulating three
...
exactly whythe multithreaded CPU evaluator dispatches them to
...
iterations.  The saving is most pronounced at shallow zoom depths, where
the Phase~A initial guess has fewer than ${\sim}b$ correct bits and the
cubic burst is fully observable (see below).
For large period~$p$, each saved forward evaluation avoids
...
Whether the cubic burst is observable in practice depends on how many
...
observable during Phase~B.  At deep zoom ($Z \gg 2^{b}$), pixel resolution
alone provides hundreds of correct bits, placing the iterate well past the
crossover before Phase~B begins; in this regime only the $+b$ quadratic
advantage is observed.
Empirical variable-precision tests at period~16045 confirm the dependence:
starting from 100~correct bits produces two cubic Halley steps before
crossover, whereas starting from 150~correct bits shows only the quadratic
$+b$ advantage from the first step onward.
...
In practice this costs roughly one extra NR iteration compared to the
CPU path for the same target precision.
...
copies of the \code{mpf\_t} data) and performs the serialisation and
