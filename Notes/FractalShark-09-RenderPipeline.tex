...
After the per-pixel iteration kernels produce an array of escape-time
counts, several post-processing stages transform this raw data into a
final displayed image.  This section covers antialiasing (down-sampling
from a supersampled buffer), the palette-based coloring system, and
iteration-count statistics gathered via GPU reduction.  The subsequent
section (\cref{sec:render-thread-pool}) describes the threading
architecture that orchestrates these stages alongside the UI.
