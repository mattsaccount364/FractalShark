...
approach.  \Cref{fig:design-decisions} summarises the three principal
...
\node[why, below=0.15cm of b2] {period-aligned,\\bounded error};
...
compute a new reference orbit centerd on a glitched location, and
...
LA~v2 (\cref{sec:la-step-forward})---this equivalence is not a coincidence; both
...
\begin{enumerate}
  ...
  \item Compute a new reference orbit centerd on a glitched pixel's
        location.
...
integration—mapping the distance $d(c)$ to a color rather than (or in
...
\begin{enumerate}
  ...
  \item Map each pixel's iteration count~$k$ to the color at palette
        index $\mathrm{CDF}(k)$.
\end{enumerate}
The effect is that all colors in the palette are utilised approximately
...
fractal rendering optimisations beyond those currently implemented in
...
equalisation---represent practical extensions that could be integrated
into the existing pipeline without fundamental architectural changes.
\Cref{sec:conclusion} summarises the overall design and reflects on the
