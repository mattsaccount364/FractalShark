...
multithreaded MPIR backends and the manually selected CUDA Ref1 backend, together
with storage and compression modes that trade memory footprint against
recomputation.  The fused CUDA Ref2 path exists in the GPU library and tests but
is not yet an application dispatch target.
...
\begin{enumerate}
  ...
  \item Chooses an algorithm from \code{m\_Perturbation\-Alg}.  In
        ...
        selection does not choose a CUDA backend.
...
The production CPU and Ref1 paths use the expanded form.  Ref2 preserves the
same component equations but fuses their linear combinations in the transform
domain, so it is not accurately described as a separate expanded-product then
addition schedule.

The CUDA Ref2 path retains the expanded equations but changes their arithmetic
schedule.  Rather than materializing the
three products before the addition stage, it constructs each final component in
the NTT domain and defers base-$2^{32}$ carry propagation until its complete sum
has been formed.  The method removes the separate high-precision add/subtract
pipeline, but not the final carry propagation required for each result;
\cref{subsec:fused-ntt-reference-step,subsec:fused-ntt-periodicity} describes
the construction, operation schedule, and periodicity test.
...
multiply and add phases.  Early termination can avoid later arithmetic, but the
...
Application dispatch to Ref1 is manual.  Its stored-output type \code{T} may be
\code{HDRFloat<float>}, \code{HDRFloat<double>}, or
\code{HDRFloat<CudaDblflt<dblflt>>}.  The path forces
\code{PExtras::Disable} and \code{ReuseMode::DontSaveForReuse}; it does not
produce a compressed or intermediate-reuse orbit.
...
In Ref1, the orbit components are \code{HpSharkFloat}, but \code{dzdcX},
\code{dzdcY}, and \code{RadiusY} are \code{HDRFloat} values.  The periodicity
decision is performed on reduced HDR magnitudes:
...
This organization isolates termination logic from the NTT multiplication and
carry-propagating addition stages while maintaining grid-wide early-exit
behavior.  The checker publishes its status through one explicit
\code{grid.sync()} per iteration.  Its cost is amortized by the subsequent
high-precision work but is not assumed to be negligible.
