...
The reference orbit is the single most expensive computation in
...
orbit}---to be computed at the full precision of the zoom depth.  At
...
This section describes how \code{RefOrbitCalc} orchestrates single-threaded and
multithreaded MPIR backends and the two manually selected CUDA implementations,
Ref1 and Ref2, together with storage and compression modes that trade memory
footprint against recomputation.  Ref1 retains the original staged
multiplication--addition schedule.  Ref2 is the faster fused implementation;
it implements the same application contract and recurrence with a different
arithmetic schedule.
...
\begin{enumerate}
  ...
  \item Chooses an algorithm from \code{m\_Perturbation\-Alg}.  In
        \code{Auto}, the zoom-factor heuristic selects
        \code{STPeriodicity} below $10^{150}$,
        \code{MTPeriodicity3} below $10^{2000}$, and
        \code{MTPeriodicity3PerturbMTHighMTMed3} thereafter.  Automatic
        selection does not choose a CUDA backend.  The manually selected
        \code{GPU} and \code{GPURef2} values dispatch to Ref1 and Ref2,
        respectively.
...
computes the application-authoritative orbit using finite, explicitly selected
precision in
...
\begin{align}
  x_{n+1} &= x_n^2 - y_n^2 + c_x, \label{eq:st-recurrence-real} \\
  y_{n+1} &= 2 x_n y_n + c_y. \label{eq:st-recurrence-imag}
\end{align}
...
the lower-precision kernels discussed previously; this difference is acceptable because the
...
The code applies an empirically chosen radius-based heuristic (no formal derivation is known): let
...
$(x_n \pm y_n)$. This factoring can reduce computational cost when multiplication and
...
The production CPU and Ref1 paths use the expanded-product schedule: Ref1
materializes and normalizes $x_n^2$, $y_n^2$, and $2x_ny_n$ before a separate
high-precision addition stage forms
\cref{eq:ref-expanded-real,eq:ref-expanded-imag}.  Ref2 implements the same
component equations through the factored nonlinear products
\begin{align}
...
propagation.  Ref2 therefore never materializes three normalized product values
or invokes the standalone high-precision add/subtract pipeline.

Fusion does not eliminate the final base-$2^{32}$ dependency.  Ref2 forms every
...
The current \code{Auto} implementation uses the zoom factor as a proxy for
required precision.  It selects \code{STPeriodicity} below $10^{150}$,
...
The \FractalShark{} GPU reference-orbit implementations
(\cref{subsec:ref-orbit-gpu}) evaluate the Mandelbrot recurrence
\(
z_{n+1} = z_n^2 + c
\)
...
Application dispatch to Ref1 or Ref2 is manual.  Their stored-output type \code{T} may be
\code{HDRFloat<float>}, \code{HDRFloat<double>}, or
\code{HDRFloat<CudaDblflt<dblflt>>}.  Both paths force
\code{PExtras::Disable} and \code{ReuseMode::DontSaveForReuse}; neither produces
a compressed or intermediate-reuse orbit.  These are common policy
constraints of the two CUDA application paths rather than missing Ref2
integration.
...
In both Ref1 and Ref2, the orbit components are \code{HpSharkFloat}, but the
...
step, before the implementation-specific high-precision stages compute
\(z_{n+1}\). In both kernels, a single distinguished thread
...
This organization isolates termination logic from either the Ref1 staged
multiply/add pipeline or the Ref2 fused recurrence pipeline while maintaining
grid-wide early-exit behavior.  The checker publishes its status through one
explicit \code{grid.sync()} per iteration.  Ref2 uses the same publication
