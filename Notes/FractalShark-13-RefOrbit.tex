...
This section describes how \code{RefOrbitCalc} orchestrates reference
orbit computation across three backends---single-threaded CPU,
multithreaded CPU, and GPU---and the storage and compression modes that
trade memory footprint against recomputation.
...
\begin{enumerate}
  \item Picks an initial guess \((c_x,c_y)\) (center of the current view
        if unset).
  \item Chooses an algorithm (\code{ST}, \code{MT}, reuse-based
        hybrids, or~\code{GPU}) based on
        \code{m\_Perturbation\-Alg} and zoom factor heuristics.
...
computes the authoritative orbit directly using
...
The experimental \code{ReferenceReferenceOrbit2} path retains the expanded
equations but changes their arithmetic schedule.  Rather than materializing the
...
bounded reconstruction error. The approach stores only a sparse set of
...
user-controlled tolerance.  Whenever the error exceeds the threshold, the compressor emits a waypoint at \(n\)
...
multithreaded perturbation strategy (e.g.,
...
In this implementation, the derivative accumulators \code{dzdcX} and
