...
behavior across CPU and GPU backends, and efficient execution on highly
...
multiplication, both inputs are padded with zeros to length $N \ge 2n$ so that the
...
exists an $N$-th root of unity in $\mathbb{Z}_p$. This divisibility enables power-of-two NTTs
...
The test-only CPU implementation \code{ReferenceReferenceOrbit2} applies the
same NTT machinery to the complete Mandelbrot recurrence, rather than using an
NTT multiplication stage followed by a distinct high-precision addition stage.
Let
\begin{equation}
  z_n = x_n + i\,y_n
  \quad\text{and}\quad
...
The fused implementation forward-transforms the limb streams for $x_n$, $y_n$,
$a$, and $b$.  It constructs the two output spectra by pointwise accumulation
of the corresponding products and exponent-aligned linear terms:
\begin{align}
  \widehat{X}_{n+1} &=
...
$b=16$ be the packed coefficient width, let $L=\lceil32D/b\rceil$, and let
$M$ be the power-of-two transform length.  The fixed plan satisfies
$M\ge2L$ and, because it is the next power of two, $M=\Theta(D)$.  Thus
...
\begin{tikzpicture}[
  ...
  \node[fused stage] (forward) {
    $\begin{gathered}
      ...
      A=\mathcal{T}_M(a),\quad B=\mathcal{T}_M(b)
    \end{gathered}$\\[-1mm]
    four forward NTTs; $\mathcal{O}(M\log M)$};
...
\cref{subsec:fused-ntt-range} holds.  The four transforms are performed for
every iteration in the present CPU prototype, including the transforms of the
constant components $a$ and $b$.

With the first Newton--Raphson derivative, write
$u_n=d_{x,n}$ and $v_n=d_{y,n}$, and form
\begin{equation}
  U=\mathcal{T}_M(u_n),\qquad V=\mathcal{T}_M(v_n),\qquad E=\mathcal{T}_M(1).
...
Thus, the derivative extension adds three forward NTTs, four pointwise products,
...
is the sum of the transforms.  Consequently, \code{AccumulateOutputSpectrum}
...
an inverse transform.  \code{AddShiftedSpectrum} performs those signed,
exponent-aligned additions and subtractions modulo the NTT prime.  They are
...
linearly shifted $c$ term.  Those conditions prevent cyclic convolution and
bound an unshifted product, but they alone do not prove
\eqref{eq:fused-ntt-no-alias-bound} for a sum of exponent-aligned terms.  The
prototype restricts product alignment to fewer than 16 bits and the linear
term to one mantissa width plus or minus those 16 bits.  Exactness consequently
depends on evaluating \eqref{eq:fused-ntt-component-bound} for the permitted
term shifts and chosen plan.  The bound is a required implementation invariant;
modular field arithmetic by itself cannot establish it.
...
\hline
Orbit only & 4 & 3 & 5 & 2 & 3 & 6 & 2 \\
Orbit plus first derivative & 7 & 7 & 10 & 4 & 6 & 13 & 4 \\
\hline
\end{tabular}
\end{table}
...
Each insertion still has a cost.  \code{AddShiftedSpectrum} applies the
exponent shift and sign by modular scaling and a modular field addition or
...
produced normalized $x_n$ and $y_n$.  The periodicity-enabled CPU path also
retains an HDRFloat derivative estimate
...
The original \code{ReferenceReferenceOrbit} path first materializes and
...
high-precision values.  This scheduling can reduce intermediate storage and
normalization work, but it is a CPU test implementation that rebuilds NTT state
for each step.  Its performance relative to the original path consequently
requires measurement rather than inference from the arithmetic alone.
...
64-bit rolling checksums are employed (e.g., CRC-64 or Fletcher-64) because they are
...
specialization from a fixed supported set (\(\{256,512,\dots,524288\}\) bits).Each invocation appends the emitted
