...
The application-facing GPU reference-orbit backend is a stateful,
precision-specialized execution engine.  It advances the recurrence
\[
...
and termination.  In this chapter, \emph{Ref1} denotes that application path.
\emph{Ref2} denotes the fused CUDA implementation described in
\cref{subsec:fused-ntt-reference-step}; it is implemented and exercised by the
GPU test library, but application dispatch does not currently select it.

\begin{table}[!htbp]
\centering
\caption[Reference-orbit backend status]{Reference-orbit backend status in the
current codebase.  ``Application'' means that \code{RefOrbitCalc} can dispatch
to the backend; ``test'' includes correctness and performance coverage in the
GPU library.}
\label{tab:reference-backend-status}
\small
\begin{tabularx}{\textwidth}{lXXl}
\hline
\textbf{Backend} & \textbf{Arithmetic} & \textbf{Current role} & \textbf{Selected by Auto} \\
\hline
Single-/multithreaded CPU & MPIR & Application reference path & Yes \\
CUDA Ref1 & \code{HpSharkFloat} & Application and tests & No \\
CUDA Ref2 & Fused fixed-width NTT & GPU library and tests & No \\
...
\code{PerturbationResults}. The GPU owns the Ref1 numerical state, executes its
high-precision arithmetic pipeline, detects escape and periodicity, and reports
...
real arithmetic.  Like every fixed-width backend, Ref1 discards information
below its retained significand window.
...
this section. The discussion begins with the underlying \code{HpSharkFloat}
representation and then describes the internal arithmetic pipelines used within
...
the recurrence itself: algebraic factorization reduces the orbit to two
frequency-domain output streams, and enabling the first Newton--Raphson
derivative raises the total to four.  Shared-memory microkernels, paired stages,
and asynchronous twiddle staging then advance those streams together, as
described in \cref{subsec:amortize-sync}.

\subsection{Fused NTT Reference-Orbit Step}
...
\begin{tikzpicture}[
  ...
  \node[fused stage, below=of inverse] (finalize) {
    \textbf{Finalization:} one base-$2^{32}$ carry/borrow and normalization
    stream for each of the two orbit or four orbit-and-derivative components};
...
Montgomery products, two inverse NTTs, and two final carry streams.  The complete
iteration uses four forward NTTs, five pointwise products, four inverse NTTs,
and four final carry streams.
...
linear significand contribution at its independently planned bit offset.

The resulting signed base-$2^{32}$ limbs are not yet a normalized
\code{HpSharkFloat}.  A decoupled-look-back carry pass propagates carries and
borrows across the complete component, after which finalization selects the
normalized mantissa window and exponent.  Combining the nonlinear and linear
terms before that pass is the relevant fusion: the orbit needs one carry stream
for $x_{n+1}$ and one for $y_{n+1}$, while the derivative-enabled path needs one
for each of its four completed components.  No algebraic rearrangement of the
field transform can remove those final positional dependencies.
...
\begin{table}[!htbp]
\centering
\caption[Ref2 recurrence operation counts]{Production Ref2 per-iteration
operation counts.  $F$ and $I$ count forward and inverse NTTs, $P$ counts
pointwise Montgomery products per frequency, and $C$ counts final
base-$2^{32}$ carry/normalization streams.  Linear inputs are fused during
signed unpack and require no transform.}
\label{tab:fused-reference-operation-counts}
\begin{tabular}{lrrrrl}
\hline
\textbf{Workload} & $F$ & $P$ & $I$ & $C$ & \textbf{Linear inputs} \\
\hline
Orbit only & 2 & 2 & 2 & 2 & $a,b$ \\
Orbit plus first derivative & 4 & 5 & 4 & 4 & $a,b,1$ \\
\hline
\end{tabular}
\end{table}
...
nonlinear contributions before its sole carry stream.  The HDRFloat
...
ordinary signed values until unpack.  Application-level performance relative to
Ref1 still requires measurement.

\subsection{Batched Transforms and Central Shared-Memory Fusion}
...
The final decoupled-look-back carry phase is likewise batched across the two or
four completed components.  Descriptor initialization is scheduled against an
...
inverse normalization, and performs only one carry traversal per final
component.  The relative contribution of each saving remains an empirical
...
iteration at the selected fixed precision to locate periodic points
