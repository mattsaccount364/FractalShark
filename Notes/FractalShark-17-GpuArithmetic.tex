...
$P$ counts high-precision NTT convolution products, $A$ counts mathematical
binary add/subtract combinations, and $C$ counts logical base-$2^{32}$
carry-propagation and normalization streams.  NTT butterfly additions are
modular field operations and are excluded from $C$.}
\label{tab:fused-reference-operation-counts}
\begin{tabular}{lrrrr}
\hline
\textbf{Workload} & $P$ & $A$ & $C_{\mathrm{Ref1}}$ & $C_{\mathrm{Ref2}}$ \\
\hline
Orbit only & 3 & 3 & 6 & 2 \\
Orbit plus first derivative & 7 & 6 & 13 & 4 \\
\hline
\end{tabular}
\end{table}
...
subtraction at every frequency index.  That work is $O(N)$ and is not free.
