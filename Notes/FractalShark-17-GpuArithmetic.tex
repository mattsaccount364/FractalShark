...
same NTT length and modular-arithmetic parameters, multiway helpers advance the
active channels through common phase boundaries.  Shared-memory microkernels,
paired stages, and asynchronous staging can use different schedules within a
phase.  This batching strategy, described in
\cref{subsec:amortize-sync}, amortizes the cost of grid-wide
synchronization across all simultaneously computed products and is
...
The Ref2 CUDA path, centered on \code{GpuOrbitSession2}, applies NTT linearity to
the complete Mandelbrot recurrence rather than following an NTT multiplication
with a distinct high-precision addition.  Setup, loop, prepared-plan ownership,
and generated precision specializations are implemented in the GPU library and
covered by its tests.  \code{ReferenceReferenceOrbit2} is the corresponding host
correctness model.  Ref2 is not currently wired into \code{RefOrbitCalc}
application dispatch, as summarized in \cref{tab:reference-backend-status}.
Let
\begin{equation}
  z_n = x_n + i\,y_n
  \quad\text{and}\quad
...
The setup kernel prepares spectra for the constant components $a$ and $b$ for
every cached active plan.  Each iteration forward-transforms only the changing
limb streams $x_n$ and $y_n$, then constructs the two output spectra by
pointwise accumulation of products and exponent-aligned constant spectra:
\begin{align}
  \widehat{X}_{n+1} &=
      \widehat{x_n}\,\widehat{x_n}
      - \widehat{y_n}\,\widehat{y_n}
      + \widehat{a},
  \label{eq:fused-reference-real-spectrum} \\
  \widehat{Y}_{n+1} &=
      2\,\widehat{x_n}\,\widehat{y_n}
      + \widehat{b}.
  \label{eq:fused-reference-imag-spectrum}
\end{align}
Here hats denote the twisted NTT-domain representation, and multiplication
denotes pointwise multiplication.  Each spectrum is then inverse-transformed,
converted to signed limbs, carry-propagated, and normalized once to produce its
final \code{HpSharkFloat} component.  The exponent alignment in
\cref{eq:fused-reference-real-spectrum,eq:fused-reference-imag-spectrum} is
essential because the product and linear terms need not share a common binary
exponent before they are accumulated.

\subsubsection{Formal Operation Schedule}
\label{subsec:fused-ntt-operation-schedule}

For this schedule, let $D$ be the number of 32-bit significand limbs, let
$b=16$ be the packed coefficient width, let $L=\lceil32D/b\rceil=2D$, and let
$M$ be the active power-of-two transform length.  The base plan has $M\ge2L$,
but a fused step selects a common minimum exponent, computes the
whole-coefficient support implied by all term shifts, and increases $M$ to the
next power of two when necessary.  The workspace supports active lengths up to
$2^{25}$ coefficients; an iteration whose required support exceeds that ceiling
returns an unknown/error status.  For a fixed shift envelope,
$\mathcal{O}(M)=\mathcal{O}(D)$ and
$\mathcal{O}(M\log M)=\mathcal{O}(D\log D)$.  This notation deliberately
reserves $M$ for the transform length:
$N$ already denotes the generic multi-precision limb count in
\cref{sec:notation}.  A nonnegative significand is packed as
\begin{equation}
  q=\sum_{j=0}^{L-1}q_j2^{bj},
  \qquad 0\le q_j<2^b.
  \label{eq:fused-ntt-coefficient-packing}
\end{equation}
Let $\mathcal{T}_M$ denote the complete packed, twisted, Montgomery-domain
forward NTT, and let $\mathcal{T}_M^{-1}$ include the inverse NTT, untwisting,
inverse-length scaling, and conversion out of Montgomery form.  For a spectrum
$Q$, define the exponent-alignment map
\begin{equation}
  \left(\mathcal{S}_sQ\right)_k
  =2^{s\bmod b}\psi^{\lfloor s/b\rfloor(2k+1)}Q_k\pmod p,
  \qquad 0\le k<M.
  \label{eq:fused-ntt-spectrum-shift}
\end{equation}
It applies a whole-coefficient shift through the twist factor and a remaining
bit shift through the power of two.  The implementation realizes
\eqref{eq:fused-ntt-spectrum-shift} with two Montgomery multiplications per
frequency index.  The shift $s$ includes the binary factor in $2x_ny_n$.
...
\begin{tikzpicture}[
  ...
  node distance=6mm]
  \node[fused stage] (forward) {
    $\begin{gathered}
      X=\mathcal{T}_M(x_n),\quad Y=\mathcal{T}_M(y_n),\\
      A=\mathcal{T}_M(a),\quad B=\mathcal{T}_M(b)\ \text{(cached at setup)}
    \end{gathered}$\\[-1mm]
    two per-iteration forward NTTs; $\mathcal{O}(M\log M)$};

  \node[fused stage, below=of forward] (products) {
    $\begin{gathered}
      P_{xx,k}=X_k\otimes X_k,\\
      P_{yy,k}=Y_k\otimes Y_k,\\
      P_{xy,k}=X_k\otimes Y_k
    \end{gathered}$\\[-1mm]
    three pointwise Montgomery products; $\mathcal{O}(M)$};

  \node[fused stage, below=of products] (accumulate) {
    $\begin{gathered}
      \text{Real:}\quad
      \begin{aligned}
        R_k^{[0]}&=0,&
        R_k^{[1]}&=R_k^{[0]}\oplus\mathcal{S}_{s_{xx}}P_{xx,k},\\
        R_k^{[2]}&=R_k^{[1]}\ominus\mathcal{S}_{s_{yy}}P_{yy,k},&
        R_k^{[3]}&=R_k^{[2]}\oplus\mathcal{S}_{s_a}A_k;
      \end{aligned}\\[1mm]
      \text{Imaginary:}\quad
      \begin{aligned}
        I_k^{[0]}&=0,&
        I_k^{[1]}&=I_k^{[0]}\oplus\mathcal{S}_{s_{2xy}}P_{xy,k},\\
        I_k^{[2]}&=I_k^{[1]}\oplus\mathcal{S}_{s_b}B_k.
      \end{aligned}
    \end{gathered}$\\[-1mm]
    five independent spectrum insertions; $\mathcal{O}(M)$};

  \node[fused stage, below=of accumulate] (inverse) {
    $\begin{gathered}
      r=\mathcal{T}_M^{-1}(R^{[3]}),\\
      i=\mathcal{T}_M^{-1}(I^{[2]})
    \end{gathered}$\\[-1mm]
    two inverse NTTs; $\mathcal{O}(M\log M)$};

  \node[fused stage, below=of inverse] (finalize) {
    $\begin{aligned}
      x_{n+1}&=\operatorname{Normalize}_{2^{32}}
        \!\left(\operatorname{Carry}_{2^{32}}
        \!\left(\operatorname{Unpack}_{b\mathbin{\to}32}(r)\right)\right),\\
      y_{n+1}&=\operatorname{Normalize}_{2^{32}}
        \!\left(\operatorname{Carry}_{2^{32}}
        \!\left(\operatorname{Unpack}_{b\mathbin{\to}32}(i)\right)\right)
    \end{aligned}$\\[-1mm]
    two signed carry/normalization streams; $\mathcal{O}(D)$};

  \draw[fused arrow] (forward) -- (products);
  \draw[fused arrow] (products) -- (accumulate);
  \draw[fused arrow] (accumulate) -- (inverse);
  \draw[fused arrow] (inverse) -- (finalize);
\end{tikzpicture}
\caption[Fused NTT orbit operation schedule]{Formal Ref2 orbit-only operation
schedule.  $\otimes$, $\oplus$, and $\ominus$ denote Montgomery multiplication,
addition, and subtraction in $\mathbb{Z}_p$, respectively.  Each occurrence of
$\mathcal{S}$ is one exponent-aligned spectrum insertion.  The real and
imaginary recurrence components remain separate until their independently
finalized high-precision values form $z_{n+1}=x_{n+1}+i\,y_{n+1}$.}
\label{fig:fused-ntt-reference-operation-schedule}
\end{figure}

\Cref{fig:fused-ntt-reference-operation-schedule} is an operation schedule,
not an algebraic approximation.  The three product spectra and all five linear
spectrum insertions are exact field operations when the range condition in
\cref{subsec:fused-ntt-range} holds.  The CUDA setup kernel caches $A$ and $B$;
the two orbit transforms are repeated each iteration.

With the first Newton--Raphson derivative, write
$u_n=d_{x,n}$ and $v_n=d_{y,n}$, and form
\begin{equation}
  U=\mathcal{T}_M(u_n),\qquad V=\mathcal{T}_M(v_n),\qquad
  E=\mathcal{T}_M(1)\ \text{(cached at setup)}.
  \label{eq:fused-ntt-derivative-forward}
\end{equation}
The two derivative components are accumulated by the same mathematical
schedule:
\begin{align}
  D_{x,k}^{[0]} &= 0, \notag\\
  D_{x,k}^{[1]} &= D_{x,k}^{[0]}\oplus\mathcal{S}_{s_{2xu}}(X_k\otimes U_k), \notag\\
  D_{x,k}^{[2]} &= D_{x,k}^{[1]}\ominus\mathcal{S}_{s_{2yv}}(Y_k\otimes V_k), \notag\\
  D_{x,k}^{[3]} &= D_{x,k}^{[2]}\oplus\mathcal{S}_{s_1}E_k, &
  d_{x,n+1} &=
    \operatorname{Finalize}\!\left(\mathcal{T}_M^{-1}(D_x^{[3]})\right).
  \label{eq:fused-ntt-derivative-real-schedule}
\end{align}
\begin{align}
  D_{y,k}^{[0]} &= 0, \notag\\
  D_{y,k}^{[1]} &= D_{y,k}^{[0]}\oplus\mathcal{S}_{s_{2yu}}(Y_k\otimes U_k), \notag\\
  D_{y,k}^{[2]} &= D_{y,k}^{[1]}\oplus\mathcal{S}_{s_{2xv}}(X_k\otimes V_k), &
  d_{y,n+1} &=
    \operatorname{Finalize}\!\left(\mathcal{T}_M^{-1}(D_y^{[2]})\right).
  \label{eq:fused-ntt-derivative-imag-schedule}
\end{align}
Here $\operatorname{Finalize}$ denotes the unpack, signed carry propagation,
and normalization displayed in \cref{fig:fused-ntt-reference-operation-schedule}.
Thus, the derivative extension adds two per-iteration forward NTTs, four pointwise products,
five spectrum insertions, two inverse NTTs, and two carry/normalization streams.

\subsubsection{Why Fusion Does Not Eliminate the Final Carry}
\label{subsec:fused-ntt-carry}

Fusion is valid because the NTT is linear over its prime field.  After each
term has been shifted to the selected common exponent, the transform of a sum
is the sum of the transforms.  Consequently, \code{AccumulateFixedOutputSpectra}
can form a component such as \cref{eq:fused-reference-real-spectrum} by
combining the pointwise product spectra with the transformed linear term before
an inverse transform.  \code{ScaleSpectrumCoefficient} supplies the modular
scale used for the signed, exponent-aligned additions and subtractions modulo
the NTT prime.  They are
field operations, not high-precision base-$2^{32}$ additions, and therefore do
not require a cross-limb carry or borrow pass at that point.

The inverse NTT does not produce a normalized \code{HpSharkFloat} directly.
Its coefficients are unpacked into overlapping signed limb contributions.  The
inverse-to-signed-limb, signed-limb propagation, and normalization stages first
reconstruct those signed limbs, then perform the necessary base-$2^{32}$ carry
and borrow propagation, and finally select the normalized mantissa window and
exponent.  Thus, Ref2 does not eliminate carry propagation; it performs exactly
one final carry/normalization pass for each completed output component, after
every product and linear term for that component has already been combined.
...
These limbs require carry and borrow propagation only in the two
$\operatorname{Finalize}$ operations shown in
...
The more important bound concerns recovery of an ordinary signed integer
coefficient after the inverse NTT.  Let $B=2^b$, let $r_t=s_t\bmod b$, and
let $\mathcal{P}$ and $\mathcal{L}$ be the product and linear terms,
respectively, that contribute to one completed component.  Before reduction
modulo $p$, the magnitude of every packed coefficient is bounded by
\begin{equation}
  \left|w_j\right|
  \le B_{\mathrm{component}}
  =\sum_{t\in\mathcal{P}}2^{r_t}L(B-1)^2
   +\sum_{t\in\mathcal{L}}2^{r_t}(B-1).
  \label{eq:fused-ntt-component-bound}
\end{equation}
The whole-coefficient part $\lfloor s_t/b\rfloor$ changes the location of a
term but not the coefficient magnitude; only $r_t$ scales a coefficient.
For example, the real orbit component has two product terms and one linear
term, while the imaginary component has one product term, whose $s_{2xy}$
already includes the factor~2, and one linear term.

Signed unpacking interprets residues above $(p-1)/2$ as negative.  Therefore
the sufficient no-aliasing condition is
\begin{equation}
  B_{\mathrm{component}} < \frac{p-1}{2}
  = 2^{63}-2^{31}.
  \label{eq:fused-ntt-no-alias-bound}
\end{equation}
Under \eqref{eq:fused-ntt-no-alias-bound}, reduction modulo $p$ is injective
on every possible signed coefficient in the schedule, so inverse-transform
residues reconstruct the intended signed limbs.  If the bound is violated,
the NTT and Montgomery arithmetic can remain perfectly valid while two
different integer coefficients share one field residue; signed unpacking would
then recover an aliased value.  Such aliasing is a semantic range failure, not
a native 64-bit or 128-bit overflow.

The plan builder enforces the ordinary single-product headroom condition
\begin{equation}
  2b+\log_2M\le64,
  \label{eq:fused-ntt-plan-headroom}
\end{equation}
and Ref2 additionally asserts a no-wrap transform length $M\ge2L$ for the
linearly shifted $c$ term.  Those conditions prevent negacyclic wraparound and
bound an unshifted product, but they alone do not prove
\eqref{eq:fused-ntt-no-alias-bound} for a sum of exponent-aligned terms.  Ref2
handles arbitrary whole 16-bit coefficient shifts by selecting a sufficiently
large active plan, subject to the $2^{25}$ workspace ceiling, and handles the
remaining shift in the range $0$ through $15$ bits by modular scaling.  The CUDA
path enforces polynomial support at run time, but it does not separately check
the signed coefficient-magnitude bound in
\eqref{eq:fused-ntt-no-alias-bound}.  That bound remains a required numeric
precondition and a validation gap; modular field arithmetic alone cannot
establish it.

\begin{table}[!htbp]
\centering
\caption[Reference-orbit operation schedule]{Per-iteration operation counts.
$F$ and $I$ count forward and inverse NTTs, $P$ counts pointwise
high-precision products, $S$ counts exponent-aligned spectrum insertions, and
$A$ counts mathematical binary add/subtract combinations.  $C$ counts logical
base-$2^{32}$ carry-propagation and normalization streams.  NTT butterfly and
spectrum additions are modular field operations and are excluded from $C$.}
\label{tab:fused-reference-operation-counts}
\begin{tabular}{lrrrrrrr}
\hline
\textbf{Workload} & $F$ & $P$ & $S$ & $I$ & $A$ & $C_{\mathrm{Ref1}}$ & $C_{\mathrm{Ref2}}$ \\
\hline
Orbit only & 2 & 3 & 5 & 2 & 3 & 6 & 2 \\
Orbit plus first derivative & 4 & 7 & 10 & 4 & 6 & 13 & 4 \\
\hline
\end{tabular}
\end{table}

For the orbit alone, both implementations retain the same three expensive
high-precision products: $x_n^2$, $y_n^2$, and $x_ny_n$.  Both also retain the
same three mathematical binary combinations: two in
$x_n^2-y_n^2+a$ and one in $2x_ny_n+b$.  Ref2 does not remove those operations;
it changes where the three combinations occur.  The real spectrum receives
three terms and the imaginary spectrum receives two, for five total term
insertions.  After each output spectrum has received its first term, the
remaining insertions perform the three nontrivial add/subtract combinations.

Each insertion still has a cost.  \code{ScaleSpectrumCoefficient} applies the
exponent shift and sign by modular scaling before a modular field addition or
subtraction at every frequency index.  That work is $O(M)=O(D)$ and is not free.
It has no cross-limb carry dependency, however, and it neither materializes an
intermediate \code{HpSharkFloat} nor invokes the separate high-precision
\code{AddHelper} pipeline.  The orbit-only schedule consequently replaces the
three product finalizations and three add-candidate carry streams of Ref1 with
two final carry streams in Ref2, avoiding four carry/normalization streams per
iteration.

With the first derivative enabled, both paths retain seven high-precision NTT
products: the three orbit products and four products for the derivative update.
The four final component expressions contain six mathematical binary
add/subtract combinations.  Ref2 accumulates ten spectrum terms: three and two
for the orbit's real and imaginary outputs, respectively, and three and two for
the derivative's real and imaginary outputs.  It then finalizes four completed
components.  Ref1 instead carries seven product outputs and six add candidates,
for thirteen logical carry streams; Ref2 uses four, avoiding nine such streams
per iteration.

The original add implementation may process corresponding orbit and derivative
candidates in the same program pass, but each candidate remains a distinct
logical carry-propagating stream.  The table consequently counts work that a
GPU implementation must resolve, rather than merely counting host function
calls.  The HDRFloat second-derivative recurrence is not represented by a
\code{HpSharkFloat} limb stream and is excluded from these counts.  The expected
benefit is therefore specifically the removal of separate carry-resolved
intermediate and add/subtract streams; its magnitude still requires GPU
measurement.
...
high-precision values.  Ref2 reuses setup-time tables, cached constant spectra,
and persistent workspace across loop invocations.  Its application-level
performance relative to Ref1 still requires measurement because the application
does not yet route reference-orbit work through Ref2.
...
coordination and memory visibility is shared while producing three
mathematically independent convolution results.  The extra pointwise work is a
small fixed number of Montgomery multiplications per frequency index; its share
of run time is configuration-dependent.
...
products concurrently. These are followed by a multiway inverse NTT, after
which unified untwisting, scaling by $N^{-1}$, and conversion out
...
In high-precision reference-orbit computation, cross terms and squared terms
arise together.  Batching them reduces duplicated phases and amortizes common
barriers.  The compile-time stream count is three for orbit computation and six
when Newton--Raphson support is enabled; their packed masks occupy 18 and
36~bits, respectively, in a 64-bit word.  A concrete use appears in
\cref{subsec:ff-inner-gpu}, where derivative products share the multiway phase
schedule.  End-to-end benefit remains a measurement question rather than a
claim that synchronization has zero cost.
