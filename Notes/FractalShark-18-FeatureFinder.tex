...
(\code{mpf\_mul\_ui} on CPU, an exponent adjustment during GPU unpack), which is
...
factor is folded into the fused signed-unpack and carry stage.  Either way the total is
...
GPU backend can batch them in one fused NTT schedule
...
A naive GPU approach would evaluate the orbit and NR derivative products in
separate cooperative launches.  \FractalShark{} instead interleaves them in
the fused persistent reference kernel.  The operands share transform stages,
pointwise work, signed unpack, carry resolution, and their grid-wide barriers.
...
\item \textbf{$d2$ accumulation (HDRFloat).}
  ...
  and the fused high-precision work does not depend on them.

\item \textbf{Fused NTT product schedule.}
  Computes all 7~products in the persistent cooperative kernel:
  \begin{center}
  \small
  \begin{tabular}{lll}
  \hline
...
\item \textbf{Signed unpack and carry resolution.}
...
\paragraph{Barrier reuse in the fused NTT schedule.}
The NTT product schedule
...
\paragraph{Barrier reuse in signed unpack and carry resolution.}
The output phase computes the orbit results ($x^2 - y^2 + c_x$ and
$2xy + c_y$) through signed coefficient accumulation, carry propagation,
and normalization.  When NR is enabled, the W0--W3 contributions are
processed beside the orbit contributions between the same synchronization
points.
...
multiply-by-two pass.  The fused recurrence records the required bit shifts
when it aligns each contribution for signed unpack.  The orbit products XX
and YY are not scaled; XY and all four W products receive the exact
$\times 2$ shift.  This layout matches the distributed form of the equations.

\paragraph{Summary: shared NR schedule.}
The orbit-only reference orbit kernel computes 3~multiplies and
2~output results per iteration.  Enabling NR adds 4~more multiplies and
2~more output results, but these are folded into the existing fused schedule
...
\begin{tikzpicture}[
  ...
  \node[font=\small\bfseries] at (5.5, -1.4) {Signed unpack/carry};
...
\caption[GPU fused NTT schedule with shared barriers]{GPU fused NTT schedule for one iteration.  The orbit
  ...
  barriers (red dashed lines).  The signed-unpack and carry phase similarly
  interleaves orbit and NR combinations.  Enabling NR adds arithmetic between barriers
