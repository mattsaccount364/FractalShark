...
iteration kernels.  This separation allows the display thread to run
antialiasing and read back the color buffer while the compute thread
...
color mapping for visual exploration without re-rendering.
...
\code{RenderThreadPool} owns \code{NumWorkers} worker threads (one per
GPU renderer, currently four) and a
...
\begin{itemize}
  ...
  \item \textbf{Coordinates.}  A \code{PointZoomBBConverter Ptz}
        snapshot captures the view center, bounding box, and zoom
  ...
  \item \textbf{Render configuration.}  \code{Algorithm},
        \code{Iter\-Type}, \code{Num\-Iterations},
        \code{Iteration\-Precision}, \code{Scrn\-Width},
        \code{Scrn\-Height}, and \code{Gpu\-Anti\-aliasing} are
        snapshotted from the \code{Fractal} at enqueue time.
        Three dirty flags (\code{Changed\-Window},
        \code{Changed\-Scrn}, \code{Changed\-Iterations}) are
        always set to \code{true} in the snapshot so that
        \code{Calc\-Fractal} does not skip computation due to
        stale clean flags from a prior render.
  ...
  \item \textbf{Completion signalling.}  A
        \code{std::shared\_ptr<std::promise<void>\hspace{0pt}>}
        (called \code{Completion\-Promise}) is signalled when the job
        finishes, whether by successful rendering, superseding, or
        error.  Callers may wait via the returned
        \code{Render\-Job\-Handle}.
\end{itemize}
...
calc-fractal mutex (if dimensions match).  This swap ensures that
...
\begin{enumerate}
  \item \code{m\_WorkQueueMutex} --- protects the work queue and
        sequence counter.
  \item \code{m\_CalcFractalMutex} --- serializes
...
antialiasing and color-extraction kernels
...
\begin{itemize}
  \item \textbf{Progressive frames.}
        ...
        synchronizes on the display stream only
  ...
  \item \textbf{Final frames.}
        ...
        synchronizes on the compute stream
        (\code{SyncComputeStream}) to guarantee that all iteration
        kernels have completed before color extraction.
...
\paragraph{Reference orbit indicators.}
\label{par:ref-orbit-indicators}
After every final frame, the render thread draws a white dot at the
screen location of each stored reference orbit's center.  The overlay
is unconditional: \code{DrawAllPerturbationResults} calls
\code{DrawPerturbationResultsHelper}, which iterates all perturbation
result vectors, converts each orbit's high-precision center
(\code{GetHiX}, \code{GetHiY}) to screen coordinates, and emits an
OpenGL \code{GL\_POINTS} vertex colored white.  When multiple
reference orbits are stored (e.g.\ after re-centering without clearing
references), multiple dots appear.  The dots are drawn with
\code{GL\_COLOR\_LOGIC\_OP} disabled, so they overwrite the fractal
image directly rather than inverting it.  The user-facing description
of this feature appears in \cref{subsec:practical-usage}.
...
\begin{enumerate}
  ...
  \item \textbf{Generation counter.}  Each supersedable enqueue increments a
        ...
        since dequeue, the stale item is skipped with a tombstone.  This check catches
...
forwarded to the GL consumer thread for texture upload.  After a successful
final frame, the worker publishes its private iteration container into
\code{m\_CurIters} when the dimensions and antialiasing factor still match.
...
\textbf{Invariant.}  Any code that requires a stable CPU-side
\code{m\_CurIters} snapshot must call \code{RenderThreadPool::Drain()} first
to ensure no pool workers are active.  Pool renders publish successful final
frames into \code{m\_CurIters}; direct renders write into \code{m\_CurIters}
for workflows that bypass the pool.

\paragraph{Data-flow trace.}
In the pool path, the data-flow sequence proceeds as follows:
\begin{enumerate}
  \item The worker thread calls \code{Acquire\-Iters\-Memory} to obtain a
        private \code{Iters\-Memory\-Container} from the pool.
  ...
  \item On compute completion, \code{ProduceFrame(isFinal\!=\!true)}
        ...
        color-mapping kernels, copies the resulting \code{Color16}
  ...
  \item The worker swaps the completed iteration container into
        \code{m\_CurIters} when the container still matches the live render
        dimensions.
  \item The GL consumer thread dequeues the frame in sequence
        order via \code{Try\-Pop\-Next\-In\-Order}, uploads the
        \code{Color16} data as a \code{GL\_RGBA16} texture, draws
        a textured quad, and returns the buffer to the pool via
        \code{Release\-Frame\-Buffer}.
\end{enumerate}
...
\code{m\_CurIters} during calculation, then publishes the completed
container after final frame production.  The direct path should be used for
workflows that intentionally bypass the pool, such as regression testing
(\code{CrummyTest}) and high-resolution saves.  Callers that inspect
\code{m\_CurIters} after pool rendering must first call \code{Drain()} to
ensure that no pool worker is concurrently modifying shared state.
...
synchronize via the display stream rather than the compute stream,
...
\code{CalcFractal} completes, synchronizes the compute stream
...
\code{mpf\_clear} falls back to the default allocator, which cannot recognize
...
plane.  All internal coordinate quantities---center, bounding-box
corners, zoom factor, radius---are stored as \code{HighPrecision}
values, which wrap MPIR's \code{mpf\_t} arbitrary-precision
floating-point type.  This representation allows the coordinate system to maintain
meaningful precision at extreme zoom depths where IEEE~754 types are
long exhausted.

\paragraph{Bounding box from center and zoom.}
Given a center $(c_x, c_y)$ and zoom factor~$Z$, the visible region
is
\[
...
\begin{itemize}
  ...
  \item \textbf{\code{HDRFloat<T>}.}  Direct cast from
        ...
        normalizes the mantissa--exponent representation so that the
...
Zooming recomputes the bounding box around the current center:
...
\begin{itemize}
  \item \textbf{Standard precision.}
        ...
        (\code{Authoritative\-Min\-Extra\-Precision\-In\-Bits}).
        The 120-bit padding provides roughly 36 decimal guard digits
        beyond the last significant bit of the per-pixel delta,
        which suffices for single-use reference orbit computation
...
\label{subsec:auto-algorithm}

When the user selects ``Auto'' mode
(\code{RenderAlgorithmEnum::AUTO}), the system chooses a rendering
algorithm based on the current zoom factor.  Four thresholds govern the
selection:
\begin{itemize}
  \item \textbf{zoom $< 10^{4}$}: \code{Gpu1x32}---plain
        single-precision iteration with no perturbation.
  \item \textbf{$10^{4} \le \text{zoom} < 10^{9}$}:
        \code{Gpu1x32PerturbedLAv2PO}---perturbation-only mode that
        skips the linear-approximation stage traversal.
  \item \textbf{$10^{9} \le \text{zoom} < 10^{34}$}:
        \code{Gpu1x32PerturbedLAv2}---the full LA~v2 pipeline.
  \item \textbf{zoom $\ge 10^{34}$}:
        \code{GpuHDRx32PerturbedLAv2}---HDR arithmetic for extreme
        zoom depths.
\end{itemize}
The $10^{34}$ boundary corresponds to the representable exponent range
of 32-bit floating-point numbers.  If the HDRx32 variant produces
visual artifacts at extreme depths, the user can manually select
\code{HDRx2x32} or \code{HDRx64} for additional precision.
...
arithmetic kernels described in \cref{subsec:ref-orbit-gpu}.  It tests
three core operations: high-precision addition
(\cref{sec:hp-add}), NTT-based multiplication
(\cref{sec:ntt-multiply}), and reference-orbit iteration.

Each operation is validated through a three-level cross-comparison
strategy:

\begin{enumerate}
  \item \textbf{GPU vs.\ MPIR.}  The GPU kernel result is converted to
        an MPIR \code{mpf\_t} and compared against the same computation
        performed entirely in MPIR arbitrary-precision arithmetic.  MPIR
        serves as the ground-truth reference.
  \item \textbf{GPU vs.\ CPU reference.}  A CPU-side reference
        implementation uses the same \code{HpSharkFloat} limb format as
        the GPU, providing a second independent check that isolates
        GPU-specific bugs from data-format issues.
  \item \textbf{CPU reference vs.\ MPIR.}  The CPU reference is itself
        compared against MPIR, validating that the reference
        implementation is trustworthy.
\end{enumerate}

\noindent
When a test fails, the checksum-guided debugging infrastructure
(\cref{sec:checksums}) narrows the fault to a specific pipeline stage.
Because each stage records a deterministic checksum on both host and
device, the first mismatching stage identifies where the GPU diverged
from the reference---reducing a diffuse ``wrong answer'' to a targeted
investigation.

Multiply tests additionally validate Newton--Raphson derivative products
(W0--W3) when the \code{EnableNewtonRaphson} parameter is active.  Test
inputs include over~25 hardcoded vectors chosen to exercise sign
combinations and boundary conditions, supplemented by randomly generated
operands.  Performance benchmarks exercise the full reference-orbit
pipeline at three zoom depths: View~5 (${\sim}16{,}000$ iterations),
View~30 (${\sim}670{,}000$ iterations), and View~32
(${\sim}30{,}000{,}000$ iterations).
