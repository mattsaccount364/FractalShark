...
would require copying large amounts of data, which is computationally
...
pressure, so the working set stays manageable even when the logical
data set far exceeds available RAM.
...
range. In both cases the implementation \emph{never} allocates a second buffer and
...
opens or creates a backing file and constructs a section object over it. The implementation uses
...
base address. On Windows, this guarantee is achieved using native section objects backed by
...
memory-mapped I/O, this combination can preserve the core advantages of the approach—stable
