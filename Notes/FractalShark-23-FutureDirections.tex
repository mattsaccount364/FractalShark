...
The \emph{Pauldelbrot criterion} formalizes glitch detection: a pixel is
...
risk of polynomial divergence).  Optimized SA variants such as NanoMB1,
...
characterized by the period~$p$ of its attracting cycle.
...
uses boundary tracing to achieve large speedups.  An overview of the
...
\begin{enumerate}
  ...
  \item Map each pixel's iteration count~$k$ to the color at palette
        index $\mathrm{CDF}(k)$.
\end{enumerate}
The effect is that all colors in the palette are utilized approximately
equally, maximizing visual contrast in the regions where most detail
exists.
...
equalization---represent practical extensions that could be integrated
