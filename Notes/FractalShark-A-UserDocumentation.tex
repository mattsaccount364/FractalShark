...
shown. The Win32 front end reports checked, enabled, and disabled states from
the current application settings and rendering state; the Linux front end uses
the same menu and command catalog, with a few conservative state rules noted
below. The menu can be invoked by mouse (\textbf{right-click}) on both
...
The native Linux application is \code{FractalSharkGuiLinux}. It uses an X11
window and native hierarchical popup menu, presents rendered frames through
OpenGL, and uses Dear~ImGui for help, file-selection, saved-location, and manual
...
ends dispatch through the shared command catalog. Some Linux menu enablement
and checked-state rules are intentionally conservative. Linux palette rotation
also has a known display limitation: it updates palette state but does not
currently recolor the presented frame, so the interactive effect is not
visible.
...
\begin{itemize}
  ...
  \item \emph{Palette Color Depth} (submenu): Controls palette selection,
    ...
    \begin{itemize}
      ...
      \item \emph{Palette Rotation} (command): On Windows, enters an interactive
      rotation mode. The palette shifts by 10~units per frame while the image
      redraws continuously. Moving the mouse more than 5~pixels from the
      starting position exits the mode and resets the palette to its original
      state. This command is useful for quickly previewing how different color
      offsets look. The Linux implementation currently changes the internal
      palette without visibly recoloring the displayed frame.
    \end{itemize}
