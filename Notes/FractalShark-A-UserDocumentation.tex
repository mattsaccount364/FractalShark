...
Version~0.53 introduced native Linux support. The native Linux application is
\code{FractalSharkGuiLinux}. It uses an X11 window with GLX/OpenGL presentation
and Dear~ImGui for the popup menu, help, file-selection, saved-location, and
manual location-entry dialogs. The Linux and Win32 applications are fully
supported and maintained at feature parity. Both dispatch through the shared
command catalog and evaluate state through the same portable implementation.
The Linux application requires a working X display and GLX/OpenGL
implementation. GPU algorithms additionally require supported NVIDIA CUDA
hardware; CPU algorithms remain available when GPU rendering is bypassed.
...
\begin{itemize}
  ...
  \item \emph{Navigate} (submenu):
    \begin{itemize}
      ...
      \item \emph{Autozoom Feature With Checkpoint Save} (\code{a}) (command):
      See \cref{par:autozoom} for details.
      \item \emph{Autozoom Feature Without Checkpoint Save}
      (\code{A}, i.e.\ \code{Shift+a}) (command): See \cref{par:autozoom} for
      details.
  ...
  \item \emph{Perturbation} (radio group submenu): Controls perturbation
    ...
    The implementation works and remains under active development, as does
    the rest of \FractalShark{}.
    \begin{itemize}
      ...
      \item \emph{GPU-Accelerated (see README)} (radio): Selects the fused
      Ref2/v2 implementation, which evaluates the complete high-precision
      reference-orbit recurrence on the GPU.  This option is a mode selection
      (radio button), not a one-shot action.  Intended for extremely deep zooms
      where CPU-based reference computation is prohibitively slow; may be
      slower than CPU at modest precision levels.  See
      \cref{subsec:ref-orbit-gpu} for details.
...
\begin{itemize}
  \item \code{a}: Autozoom using the ``Feature'' heuristic with checkpoint
        saving enabled.  The command forces the
        ...
        target.
  \item \code{A} (\code{Shift+a}): Autozoom using the same ``Feature''
        heuristic while preserving any existing Newton--Raphson checkpoint
        instead of writing a new checkpoint.
...
The \code{AutoZoomer} class provides three heuristic strategies for
automated zoom animation.  The Feature strategy has checkpoint-saving and
checkpoint-preserving command variants.  Each heuristic repeats until a
termination condition is met (all pixels at the iteration limit, user abort,
or target reached).  The render pipeline depth of
...
\begin{description}
  ...
  \item[Feature (zoom divisor~3)]
    ...
    all frames.  The checkpoint-saving variant writes the refinement
    checkpoint, and the checkpoint-preserving variant leaves the existing
...
\code{FractalSharkCli} renders Mandelbrot views without a graphical interface.
It can render once in a single process or accept repeated requests through a
persistent server. Each render can write a PNG file, print monochrome ASCII
art, print ANSI 256-color output, or combine image and console output. The
tool supports scripting, CI golden-image tests, batch rendering on headless
servers, and terminal inspection on Windows and Linux.
...
FractalSharkCli --server [--endpoint NAME] [--width W --height H]
                [--commit-cap-bytes N]
FractalSharkCli --connect [--endpoint NAME] --render-algorithm NAME
                [--out FILE.png] [--console] [--color] [options] {view-source}
...
\begin{description}
  ...
  \item[\code{-{}-commit-cap-bytes N}]
    Memory cap for growable vectors (default unlimited). Set it when starting
    a server; clients cannot change the cap for individual requests.
...
\subsubsection*{Persistent server}

Start the server in one terminal, then submit renders and stop it from another:

\begin{lstlisting}
FractalSharkCli --server --endpoint batch --width 1280 --height 720
FractalSharkCli --connect --endpoint batch --builtin-view 0 --render-algorithm Cpu64 --out frame.png
FractalSharkCli --connect --endpoint batch --shutdown
\end{lstlisting}

The server stays in the foreground and handles requests in arrival order.
Without \code{-{}-endpoint}, both processes use a per-user local endpoint:
a named pipe on Windows or a Unix-domain socket on Linux. Server width and
height supply defaults that a render request may override. The server reads
locations files and writes PNG files, so relative paths in client arguments
are resolved from the server's working directory.

The client prints the server's output and returns its status as an exit code.
It also prints a frame time covering the request and response. For server
renders, PNG encoding can continue after the client returns; a successful
response does not guarantee that the PNG is complete. The shutdown command
waits for outstanding PNG
encoders before the server exits. The full PowerShell batch example is
\code{tools/run\_cli\_server\_example.ps1}.

\subsubsection*{Implementation}

The executable parses and validates render options in single-shot and client
modes. A client forwards the render arguments or a shutdown request over a
versioned local IPC protocol. The server validates render arguments again,
processes one connection at a time, and returns a status plus captured standard
output and error. It owns one \code{Fractal} instance across requests, retaining
initialized renderer state. In single-shot mode, the executable creates a new
\code{Fractal} for the render.

Both render modes call the shared \code{RenderToPng} function in
\code{FractalSharkLib} with the caller-owned \code{Fractal}. The helper configures
the view and algorithm, then uses the direct rendering path
(\cref{subsec:two-render-paths}): \code{Drain()} followed by
\code{CalcFractal(true)} and, when PNG output is requested,
\code{SaveCurrentFractal}. Single-shot mode waits for PNG completion; server
mode starts encoding in the background so it can overlap a later render.
