...
\begin{itemize}
  ...
  \item \emph{Perturbation} (radio group submenu): Controls perturbation
    ...
    \begin{itemize}
      ...
      a diagnostic overlay for perturbation state.  Currently a stub
      (prints a TODO to the debug console).  Note, however, that white
      dots indicating reference orbit locations are drawn unconditionally
      as part of the normal render path---see the practical-usage note
      below and \cref{par:ref-orbit-indicators} for implementation
      details.
...
% -----------------------------------------------------------------
\section{FractalSharkCli: Headless Renderer}
\label{sec:fractalshark-cli}

\code{FractalSharkCli} is a command-line tool that renders a single
Mandelbrot view to a PNG file without any graphical interface.  It is
useful for scripting, CI golden-image tests, and batch rendering on
headless servers.  The tool builds on both Windows (MSVC) and Linux
(CMake + Clang).

\subsubsection*{Usage synopsis}

\begin{lstlisting}
FractalSharkCli --render-algorithm NAME --out FILE.png [options] {view-source}
