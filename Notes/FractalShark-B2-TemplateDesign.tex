...
backends use MPIR internally, while CUDA Ref1 and Ref2 use a
...
\begin{itemize}
  ...
  \item \code{NTTPlan}---the Ref1 compile-time NTT plan.  It is computed from
        \code{GlobalNumUint32} by \code{SharkNTT::BuildPlanPrime}.  Ref2 also
        has a compile-time base \code{NTTPlan2}, but its setup allocates and
        caches larger active plans so exponent-shifted polynomial support can
        select a transform length at run time.
\end{itemize}
