...
Visual~Studio~2026, MSBuild, and the NVIDIA CUDA toolkit. Linux is also a native
target built with CMake and Clang, with release binaries targeted at Ubuntu.
...
and the \code{FractalSharkGuiLinux} graphical application. Windows remains the
...
through the same portable command identifiers as the Win32 GUI. Linux-specific
menu-state and command-handler classes translate those contracts into renderer
mutations and X11 operations. This arrangement keeps algorithm selection,
...
the Win32 GUI. Some menu enablement and checked-state rules remain conservative,
and palette rotation currently changes internal state without visibly
