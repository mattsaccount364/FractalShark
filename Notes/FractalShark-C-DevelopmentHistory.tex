...
\item \textbf{Native Linux GUI.} \code{FractalSharkGuiLinux} now uses Xlib and
  ...
  shared renderer and command catalog. Full Win32 parity remains a direction
  rather than a completed claim; palette rotation still has a Linux-specific
  display limitation. Menu enablement, check marks, and radio selections are
  evaluated by the same portable implementation on both platforms.
