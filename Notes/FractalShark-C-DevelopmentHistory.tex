...
\item \textbf{Native Linux build.}  A CMake + Clang build system is introduced
  alongside the existing Visual Studio solution, including the native
  \code{FractalSharkGuiLinux} Xlib/Dear~ImGui GUI.  Linux release binaries are
  targeted at Ubuntu, and the intended direction is full Linux/Win32 parity.
  CPU-only unit tests pass on Linux.
...
\item \textbf{Linux port (in progress).}  A CMake + Clang build system is
  introduced alongside the existing Visual Studio solution, and a GTK-based
  GUI is under development.  The port is incomplete---not all features work
  yet---but the project is moving in this direction.  CPU-only unit tests
  pass on Linux.
...
\begin{itemize}
  ...
  \item \textbf{High-precision GPU arithmetic pipeline.}
  Includes a full multiply/add/subtract pipeline
  (\cref{sec:hp-add,sec:ntt-multiply}), an NTT
  implementation, parallel-prefix carry propagation, and
  HDR-style exponent tracking.
...
\paragraph{NTT-based high-precision multiply (magic prime \texorpdfstring{$2^{64} - 2^{32} + 1$}{2\^{}64 - 2\^{}32 + 1})}
...
\item Beginner guide to the number theoretic transform \cite{NTT_BeginnerGuide2024}
\end{itemize}
...
least given a decent card.  Scroll down to ``[2025-6-15](\#2025-6-15)'' 
...
if the performance actually pays off.  (One might argue the 
Mandelbrot multiplies/squares dominate the cost, but it's bugging me and fun to 
play with).

\subsubsection{2025-4-26}
...
\subsubsection{2025-1 -- What's going on with this native CUDA reference orbit calculation?}
