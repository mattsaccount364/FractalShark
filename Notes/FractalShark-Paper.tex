...
\end{description}
Construction from a \code{double} uses Knuth's error-free sum algorithm to
...
QD library by Bailey and Hida (Lawrence Berkeley National Laboratory) and
...
arithmetic (MPIR on CPU or \code{HpSharkFloat} on GPU).  Also adapted from
the QD library.
...
\texttt{Basic} (placeholder),
\texttt{Default} (rainbow transitions R$\to$Y$\to$G$\to$C$\to$B$\to$M),
\texttt{Patriotic} (red/blue),
\texttt{Summer} (red/green/cyan/white), and
\texttt{Random} (procedurally generated).
