...
\begin{itemize}
  ...
  \item \emph{Navigate} (submenu):
    \begin{itemize}
      ...
      Section~\ref{sec:feature-finder} for the underlying algorithm.
        \begin{itemize}
          ...
          mouse cursor using perturbation theory.  Requires a reference orbit and
          works at deeper zooms than Direct.
          ...
          mouse cursor using linear approximation.  Works at extreme zoom depths
          where PT alone would be too slow.
...
position, or scan the entire viewport.  See Section~\ref{sec:feature-finder}
for the algorithm details and Section~\ref{sec:ui-popup-menu} for the
...
explicitly managed integer exponent.  When the GPU reference-orbit kernel or
...
\begin{itemize}
\item \textbf{Reference orbit placement.}  A reference orbit centered at a
  nucleus is exactly periodic, eliminating long-term drift and improving the
  accuracy of perturbation-based rendering nearby.
...
Version \texttt{0.51} introduces the \textbf{Feature Finder}, a major new tool
...
\item \textbf{Feature Finder.}  Move mouse near a feature, select feature
  ...
  mini-Mandelbrot or satellite bulb---using Newton's method.  A line is drawn
...
\item \textbf{Under the hood.}  Extended the derivative math in the
  ...
  Improved precision tracking during zoom.  Added a \texttt{.clang-format}
