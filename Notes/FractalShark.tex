...
\begin{itemize}
  ...
  by a recalculation may restore a usable image.
    ...
    \begin{itemize}
      ...
      related concepts (e.g., what LAv2, BLA, perturbation, and reference
      compression mean, and when to use each).
...
explicitly managed integer exponent (see \cref{sec:hdr-float} for the full description).  When the GPU reference-orbit kernel or
...
Version \texttt{0.51} introduces the \textbf{Feature Finder} (\cref{sec:feature-finder}), a major new tool
...
\begin{itemize}
  ...
  multiply/add/subtract pipeline (\cref{sec:hp-add,sec:ntt-multiply}), a Number Theoretic Transform (NTT)
