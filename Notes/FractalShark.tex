...
\textit{These notes are a work-in-progress summary of \FractalShark{} but are
currently not in a well-edited or organized state.  Moreover, much of it is
AI-generated and thus may contain errors or inconsistencies.  That said, I've
made some effort to review and correct it, so it should be better than nothing.}
...
The popup menu is organized into logical groups.  Items fall into three distinct
behavioral categories:

\begin{description}
  \item[Radio groups] Mutually exclusive choices where exactly one option is
  active at a time (for example, selecting a render algorithm).
  \item[Toggle options] Independent on/off settings that can be enabled or
  disabled without affecting other items.
  \item[Commands] Immediate actions that execute when selected and do not
  maintain a checked state.
\end{description}

Radio groups and toggle states are derived from the current internal settings,
not from static menu state, ensuring that the menu always reflects the actual
configuration in use.

\subsection{Render Algorithm Selection}

The \emph{Choose Render Algorithm} submenu selects the core Mandelbrot rendering
strategy.  This is a radio group: selecting one algorithm automatically
deselects the others.

Available choices may include:
\begin{itemize}
  \item CPU-based reference and linear-approximation renderers
  \item Multiple CUDA-based GPU renderers
  \item High-dynamic-range and experimental kernels
\end{itemize}

On systems without a working CUDA setup, CPU renderers can be selected as a
fallback.  In the event of a CUDA initialization failure, switching to a CPU
algorithm followed by a recalculation may restore a usable image.

\subsection{Perturbation and Reference-Orbit Controls}

FractalShark supports perturbation-based rendering driven by a high-precision
reference orbit.  Menu options in this area control how the reference orbit is
computed and reused.

Typical actions include:
\begin{itemize}
  \item Selecting CPU or GPU-based reference-orbit computation
  \item Enabling or disabling experimental GPU reference-orbit paths
  \item Reusing an existing reference orbit when possible
\end{itemize}

GPU-accelerated reference orbits are intended for extremely deep zooms and may
be slower than CPU computation at modest precision levels.  They are marked as
experimental and should be used accordingly.

\subsection{Recalculation Commands}

Commands such as \emph{Recalculate} or \emph{Recalculate, Reuse Reference} trigger
an immediate re-render of the current view.

These commands do not change any persistent settings; they simply instruct the
renderer to restart computation using the current configuration.  Reusing the
reference orbit can significantly reduce recomputation time when only display
parameters change.
...
FractalShark is an experimental research project rather than a polished
end-user application.  Users should expect occasional crashes, rendering
failures, or unresponsive behavior when exercising less common menu options.
...
therefore employ \emph{rebasing}, in which the current deviation is folded into
the reference state and perturbation continues relative to this new baseline.
This process preserves accuracy while maintaining numerical stability.
...
Wine 9.0 on Linux.  There are still some quirks, but basic rendering works.
