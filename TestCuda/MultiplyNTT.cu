...
DebugInitRandom(cg::thread_block& block, curandState* state)
{
...
template <class SharkFloatParams, int RecursionDepth> struct SharedMemoryLayout {
    ...
    static constexpr int BaseSharedMemory =
...
__device__ uint32_t*
GetRecursionSyncVars(uint32_t* base_shared_data)
{
...
template <class SharkFloatParams, int RecursionDepth>
__device__ uint32_t*
GetRecursionPipelineBuffer(uint32_t* base_shared_data)
{
    if constexpr (SharkLoadAllInShared) {
        return nullptr; // No pipeline buffer needed
    } else {
        constexpr int base_offset =
            SharedMemoryLayout<SharkFloatParams, RecursionDepth>::RecursionOffset;
...
CompareDigits(const uint32_t* SharkRestrict highArray, const uint32_t* SharkRestrict lowArray)
{
...
SubtractDigitsParallel(uint32_t* SharkRestrict x_diff_abs,
                       ...
                       cg::thread_block& block)
{
    ...
    // 1) global thread id
    int tid =
        (block.group_index().x - ExecutionBlockBase) * block.dim_threads().x + block.thread_index().x;
    // 2) stride
    ...
    cg::thread_block& block)
{
    ...
    auto* SharkRestrict sharedBorrowAny =
        SharkLoadAllInShared ? x_diff_abs : &x_diff_abs[block.group_index().x];
    ...
    if constexpr (ExecutionNumBlocks > 1) {
        ...
        const int tid = (block.group_index().x - ExecutionBlockBase) * block.dim_threads().x +
        ...
        // Only active blocks (those with group_index().x in [ExecutionBlockBase,
        // ExecutionBlockBase+ExecutionNumBlocks)) participate.
...

        {
            ...
            for (int idx = blockStart + block.thread_index().x; idx < blockEnd;
        ...
        if (block.thread_index().x == block.dim_threads().x - 1) {
            ...
            // const int blockStart = (block.group_index().x - ExecutionBlockBase) *
            // block.dim_threads().x; const int blockEnd = blockStart + block.dim_threads().x; //
            // exclusive
        ...
        do {
            ...
            for (int pass = 0; pass < nmax; ++pass) {
                ...
                for (int localIdx = blockStart + block.thread_index().x; localIdx < blockEnd;
...
template <class SharkFloatParams>
__device__ SharkForceInlineReleaseOnly static void
SerialCarryPropagationThread0(uint64_t* SharkRestrict shared_data,
                              cg::grid_group& grid,
                              cg::thread_block& block,
                              ...
                              uint64_t* SharkRestrict resultYY)
{
    ...
    auto ProcessComponent =
        [](const uint64_t* final128, uint64_t* result, int total_digits) -> uint64_t {
...
template <class SharkFloatParams>
__device__ SharkForceInlineReleaseOnly static void
SerialCarryPropagation(uint64_t* SharkRestrict shared_data,
                       cg::grid_group& grid,
                       cg::thread_block& block,
                       int thread_start_idx,
                       int thread_end_idx,
                       ...
                       uint64_t* SharkRestrict /*globalCarryCheck*/)
{
    ...
    if (block.thread_index().x == 0 && block.group_index().x == 0) {
        SerialCarryPropagationThread0<SharkFloatParams>(shared_data,
                                                        grid,
                                                        block,
                                                        thread_start_idx,
                                                        thread_end_idx,
...
static __device__ int
CarrySharedToIndex(const bool PriorIndex, const int thread_idx)
{
...
template <class SharkFloatParams>
__device__ SharkForceInlineReleaseOnly static void
CarryPropagation(uint64_t* SharkRestrict shared_data,
                 cg::grid_group& grid,
                 cg::thread_block& block,
                 int thread_start_idx,
                 int thread_end_idx,
                 ...
                 uint64_t* SharkRestrict globalCarryCheck)
{
    ...
    for (int idx = thread_start_idx; idx < thread_end_idx; ++idx) {
        ...
        // Add local carry to sum_low
        auto LocalCarry =
            [](uint64_t& local_carry, uint64_t sum_low, uint64_t sum_high, uint64_t* result, int idx) {
                bool new_sum_low_negative = false;
    ...
    const auto carrySharedToIndex = CarrySharedToIndex(false, threadIndexInBlock);
    if (threadIndexInBlock == SharkFloatParams::GlobalThreadsPerBlock - 1) {
        const auto carryGlobalToIndex = CarryGlobalToIndex(false, block.group_index().x);
...
#pragma nv_diag_suppress 445
        auto LocalCarryIn = []<int XX_XY_YY>(cg::thread_block& block,
                                             const int MaxBlocks,
            ...
            local_carry = 0;
            if (block.thread_index().x == 0 && block.group_index().x > 0) {
                const auto block_carry_outs_idx =
                    CarryGlobalToIndex(true, block.group_index().x) + XX_XY_YY;
                local_carry = block_carry_outs[block_carry_outs_idx];
            } else {
                if (block.thread_index().x > 0) {
                    const auto shared_carries_idx =
                        CarrySharedToIndex(true, block.thread_index().x) + XX_XY_YY;
...
#pragma nv_diag_default 445
        ...
        LocalCarryIn.template operator()<0>(
            block, MaxBlocks, MaxThreads, local_carry_xx, block_carry_outs, shared_carries);
        LocalCarryIn.template operator()<1>(
            block, MaxBlocks, MaxThreads, local_carry_xy, block_carry_outs, shared_carries);
        LocalCarryIn.template operator()<2>(
            block, MaxBlocks, MaxThreads, local_carry_yy, block_carry_outs, shared_carries);
        ...
        const auto carrySharedToCurIndex = CarrySharedToIndex(false, block.thread_index().x);
        const auto carryGlobalToCurIndex = CarryGlobalToIndex(false, block.group_index().x);
...
#define DefineTempProductsOffsets(CallIndex)                                                            \
    ...
    constexpr auto CallOffset = Checksum_offset + AdditionalGlobalChecksumSpace +                       \
                                CallIndex * CalculateMultiplyFrameSize<SharkFloatParams>();             \
    constexpr auto TempBase = 0;                                                                        \
    constexpr auto TempBaseOffset = TempBase + CallOffset;                                              \
    constexpr auto Z0_offsetXX =                                                                        \
        TempBaseOffset + AdditionalUInt64PerFrame +                                                     \
        CalcAlign16Bytes64BitIndex(TempBaseOffset + AdditionalUInt64PerFrame); /* 0 */                  \
...
#define DefineExtraDefinitions()                                                                        \
    ...
    const int digits_per_thread =                                                                       \
        (digits_per_block + block.dim_threads().x - 1) / block.dim_threads().x;                         \
...
#define DefineCarryDefinitions()                                                                        \
    ...
    constexpr auto total_threads = SharkFloatParams::GlobalThreadsPerBlock *                            \
                                   SharkFloatParams::GlobalNumBlocks * per_thread_multiplier;           \
...
template <class SharkFloatParams, int RecursionDepth, int CallIndex, DebugStatePurpose Purpose>
__device__ SharkForceInlineReleaseOnly static void
                       ...
                       size_t arraySize)
{
    constexpr auto maxPurposes = static_cast<int>(DebugStatePurpose::NumPurposes);
    constexpr auto curPurpose = static_cast<int>(Purpose);
    debugStates[CallIndex * maxPurposes + curPurpose].Reset(record,
                                                            useConvolution,
                                                            grid,
                                                            block,
                                                            arrayToChecksum,
...
enum class ConditionalAccess { False, True };

// Unified scalar accumulator used for BOTH prologue and epilogue
template <class SharkFloatParams, ConditionalAccess Cond>
__device__ SharkForceInlineReleaseOnly static void
accumulate_scalar_span(cg::grid_group& grid,
                       cg::thread_block& block,
...
#pragma unroll
    for (int idx = i_lo; idx <= i_hi; ++idx) {
        ...
        if constexpr (Cond == ConditionalAccess::True) {
            xx_a = x_diff_abs[idx];
            xx_b = x_diff_abs[idx2];
            xy_a = x_diff_abs[idx];
            xy_b = y_diff_abs[idx2];
            yy_a = y_diff_abs[idx];
            yy_b = y_diff_abs[idx2];
        } else {
...
template <class SharkFloatParams,
          ...
          int ExecutionNumBlocks>
__device__ SharkForceInlineReleaseOnly static void
ProcessConvolutionBatch(cg::grid_group& grid,
                        cg::thread_block& block,
                        DebugMultiplyCount<SharkFloatParams>* debugMultiplyCounts,
                        ...
                        const uint32_t* y_diff_abs = nullptr)
{
    ...
    if constexpr (BatchSize == 1) {
        accumulate_scalar_span<SharkFloatParams, UseConditionalAccess>(grid,
                                                                       block,
                                                                       debugMultiplyCounts,
                                                                       i_start,
                                                                       i_end,
    ...
    if constexpr (SharkLoadAllInShared) {
        ...
        while (i + BatchSize - 1 <= i_end) {
            ...
            uint32_t ax0[BatchSize], bx0[BatchSize], ay0[BatchSize], by0[BatchSize], cy0[BatchSize],
                dy0[BatchSize];
            uint32_t ax1[BatchSize], bx1[BatchSize], ay1[BatchSize], by1[BatchSize], cy1[BatchSize],
                dy1[BatchSize];
            uint32_t ax2[BatchSize], bx2[BatchSize], ay2[BatchSize], by2[BatchSize], cy2[BatchSize],
                dy2[BatchSize];
            uint32_t ax3[BatchSize], bx3[BatchSize], ay3[BatchSize], by3[BatchSize], cy3[BatchSize],
                dy3[BatchSize];

            // scalar loaders (from shared inputs x_diff_abs/y_diff_abs or a/b + offsets)
            auto load_buf_shared = [&](int base_i,
                                       uint32_t(&ax)[BatchSize],
                                       uint32_t(&bx)[BatchSize],
                                       uint32_t(&ay)[BatchSize],
                                       uint32_t(&by)[BatchSize],
                                       uint32_t(&cy)[BatchSize],
                                       uint32_t(&dy)[BatchSize]) {
#pragma unroll
            ...
            auto compute_buf = [&](uint32_t(&ax)[BatchSize],
                                   uint32_t(&bx)[BatchSize],
                                   uint32_t(&ay)[BatchSize],
                                   uint32_t(&by)[BatchSize],
                                   uint32_t(&cy)[BatchSize],
...
#pragma unroll
            ...
            if (nFull == 1) {
                ...
                for (int r = 0; r < rounds; ++r) {
                    compute_buf(ax0, bx0, ay0, by0, cy0, dy0);
                    if (next < nf)
                        load_buf_shared(i + next * BatchSize, ax0, bx0, ay0, by0, cy0, dy0), ++next;

                    compute_buf(ax1, bx1, ay1, by1, cy1, dy1);
                    if (next < nf)
                        load_buf_shared(i + next * BatchSize, ax1, bx1, ay1, by1, cy1, dy1), ++next;

                    compute_buf(ax2, bx2, ay2, by2, cy2, dy2);
                    if (next < nf)
                        load_buf_shared(i + next * BatchSize, ax2, bx2, ay2, by2, cy2, dy2), ++next;

                    compute_buf(ax3, bx3, ay3, by3, cy3, dy3);
                    if (next < nf)
                        load_buf_shared(i + next * BatchSize, ax3, bx3, ay3, by3, cy3, dy3), ++next;
                }
                if (rem >= 1)
                    compute_buf(ax0, bx0, ay0, by0, cy0, dy0);
                if (rem >= 2)
    ...
    if constexpr (SharkInnerLoopOption == InnerLoopOption::TryUnalignedLoads2 ||
                  SharkInnerLoopOption == InnerLoopOption::TryUnalignedLoads2Shared) {

        ProcessConvolutionDirectLoad_Unaligned2<SharkFloatParams,
                                                16,
                                                UseConditionalAccess,
                                                RecursionDepth,
                                                ExecutionBlockBase,
                                                ExecutionNumBlocks>(grid,
                                                                    block,
                                                                    debugMultiplyCounts,
                                                                    i,
                                                                    i_start,
    ...
    if constexpr (SharkInnerLoopOption == InnerLoopOption::TryVectorLoads) {
        ...
        ProcessConvolutionDirectLoad_BS8_FwdAligned<SharkFloatParams,
                                                    8,
                                                    UseConditionalAccess,
                                                    RecursionDepth,
                                                    ExecutionBlockBase,
    ...
    if constexpr (SharkInnerLoopOption == InnerLoopOption::TryUnalignedLoads) {
        ...
        ProcessConvolutionDirectLoad_Unaligned<SharkFloatParams,
                                               8,
                                               UseConditionalAccess,
                                               RecursionDepth,
                                               ExecutionBlockBase,
...
template <class SharkFloatParams,
          ...
          int TempBase>
__device__ SharkForceInlineReleaseOnly static void
MultiplyDigitsOnly(uint32_t* SharkRestrict shared_data,
                   ...
                   uint64_t* SharkRestrict tempProducts)
{
    ...
    constexpr bool UseConvolutionBool =
        (NewNumBlocks <=
             std::max(SharkFloatParams::GlobalNumBlocks / SharkFloatParams::ConvolutionLimit, 1) ||
         (NewNumBlocks % 3 != 0));
    constexpr UseConvolution UseConvolutionHere =
        UseConvolutionBool ? UseConvolution::Yes : UseConvolution::No;
    ...
    auto* SharkRestrict subtractionBorrows =
        reinterpret_cast<uint32_t*>(&tempProducts[SubtractionOffset1]);
    auto* SharkRestrict subtractionBorrows2 =
        reinterpret_cast<uint32_t*>(&tempProducts[SubtractionOffset2]);
    auto* SharkRestrict subtractionBorrows3 =
        reinterpret_cast<uint32_t*>(&tempProducts[SubtractionOffset3]);
    auto* SharkRestrict subtractionBorrows4 =
        reinterpret_cast<uint32_t*>(&tempProducts[SubtractionOffset4]);
    auto* SharkRestrict globalBorrowAny = reinterpret_cast<uint32_t*>(&tempProducts[BorrowGlobalOffset]);
    auto* SharkRestrict globalBlockBorrow1 =
        reinterpret_cast<uint32_t*>(&tempProducts[BorrowBlockLevelOffset1]);
    auto* SharkRestrict globalBlockBorrow2 =
        reinterpret_cast<uint32_t*>(&tempProducts[BorrowBlockLevelOffset2]);
    ...
    if constexpr (!SharkFloatParams::DisableSubtraction) {
        if constexpr (UseParallelSubtract) {
            ...
            if (x_compare >= 0 && y_compare >= 0) {
                x_diff_sign = 0;
                y_diff_sign = 0;
                SubtractDigitsParallelImproved3<SharkFloatParams,
                                                n2,
                                                n1,
                                                n2,
                                                n1,
                                                ExecutionBlockBase,
                                                ExecutionNumBlocks>(x_diff_abs,
                                                                    y_diff_abs,
                                                                    a_high,
                                                                    a_low,
                                                                    b_high,
    ...
    constexpr auto FinalZ0Size =
        (UseConvolutionHere == UseConvolution::Yes) ? (total_k * 2) : (SubNewNRoundUp * 2 * 2);
    constexpr auto FinalZ2Size =
        (UseConvolutionHere == UseConvolution::Yes) ? (total_k * 2) : (SubRemainingNewN * 2 * 2);
    ...
    if constexpr (UseConvolutionHere == UseConvolution::Yes) {
        ...
        if constexpr (SharkLoadAllInShared) {
            cg::memcpy_async(block,
                             const_cast<uint32_t*>(x_diff_abs),
                             global_x_diff_abs,
                             sizeof(uint32_t) * MaxHalfN);
            cg::memcpy_async(block,
                             const_cast<uint32_t*>(y_diff_abs),
                             global_y_diff_abs,
                             sizeof(uint32_t) * MaxHalfN);
        }
        ...
        for (int idx = tid; idx < total_k * 3; idx += stride) {
            ...
            if (idx < total_k) {
                ...
                ProcessConvolutionBatch<SharkFloatParams,
    ...
    if constexpr (!SharkFloatParams::DisableAllAdditions) {
        ...
        for (int i = tid; i < total_k; i += stride) {
            ...
            if (z1_signXX == 0) {
                ...
                Subtract128(
                    xx_temp_low, xx_temp_high, xx_z1_temp_low, xx_z1_temp_high, xx_z1_low, xx_z1_high);
            } else {
            ...
            if (z1_signXY == 0) {
                // same sign: Z1 = (Z2 + Z0) - Z1_temp
                Subtract128(
                    xy_temp_low, xy_temp_high, xy_z1_temp_low, xy_z1_temp_high, xy_z1_low, xy_z1_high);
            } else {
                // opposite signs: Z1 = (Z2 + Z0) + Z1_temp
                Add128(
                    xy_temp_low, xy_temp_high, xy_z1_temp_low, xy_z1_temp_high, xy_z1_low, xy_z1_high);
            ...
            if (z1_signYY == 0) {
                // same sign: Z1 = (Z2 + Z0) - Z1_temp
                Subtract128(
                    yy_temp_low, yy_temp_high, yy_z1_temp_low, yy_z1_temp_high, yy_z1_low, yy_z1_high);
            } else {
                // opposite signs: Z1 = (Z2 + Z0) + Z1_temp
                Add128(
                    yy_temp_low, yy_temp_high, yy_z1_temp_low, yy_z1_temp_high, yy_z1_low, yy_z1_high);
        ...
        if constexpr (SharkDebugChecksums) {
            ...
            StoreCurrentDebugState<SharkFloatParams,
                                   RecursionDepth,
                                   CallIndex,
                                   DebugStatePurpose::Final128XX>(record,
                                                                  UseConvolutionHere,
                                                                  debugStates,
                                                                  grid,
                                                                  block,
                                                                  final128XX,
                                                                  total_result_digits * 2);
            StoreCurrentDebugState<SharkFloatParams,
                                   RecursionDepth,
                                   CallIndex,
                                   DebugStatePurpose::Final128XY>(record,
                                                                  UseConvolutionHere,
                                                                  debugStates,
                                                                  grid,
                                                                  block,
            ...
            StoreCurrentDebugState<SharkFloatParams,
                                   RecursionDepth,
                                   CallIndex,
                                   DebugStatePurpose::Final128YY>(record,
                                                                  UseConvolutionHere,
                                                                  debugStates,
                                                                  grid,
                                                                  block,
...
// static constexpr int32_t SharkFloatParams::GlobalNumUint32 = SharkFloatParams::GlobalThreadsPerBlock *
                                   ...
                                   uint64_t* SharkRestrict tempProducts)
{
    ...
    static_assert(FrameEnd <= TempBaseOffset + CalculateMultiplyFrameSize<SharkFloatParams>(),
                  "Per-frame buffers overflow CalculateMultiplyFrameSize");

    // Also ensure we never intrude into the global region
    static_assert(TempBaseOffset >= (Checksum_offset + AdditionalGlobalChecksumSpace),
    ...
    static_assert(kSlotsUsedNewN <= ScratchMemoryArraysForMultiply,
    ...
    auto* SharkRestrict aDigits =
        SharkLoadAllInShared ? (shared_data) : const_cast<uint32_t*>(A->Digits);
    auto* SharkRestrict bDigits = SharkLoadAllInShared
    ...
    auto* SharkRestrict y_diff_abs =
        SharkLoadAllInShared ? reinterpret_cast<uint32_t*>(x_diff_abs + (NewN + 1) / 2 +
                                                           CalcAlign16Bytes32BitIndex((NewN + 1) / 2))
    ...
    if constexpr (SharkDebugChecksums) {
        ...
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Invalid>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::ADigits>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::BDigits>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::CDigits>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::DDigits>(
        ...
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::AHalfLow>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams,
                               RecursionDepth,
                               CallIndex,
                               DebugStatePurpose::BHalfHigh>(record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::BHalfLow>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::XDiff>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::YDiff>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z0XX>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z0XY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z0YY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z1XX>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z1XY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z1YY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z2XX>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z2XY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z2YY>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z2_Perm1>(
            record, debugStates, grid, block);
        EraseCurrentDebugState<SharkFloatParams, RecursionDepth, CallIndex, DebugStatePurpose::Z2_Perm2>(
    ...
    if constexpr (!SharkFloatParams::DisableCarryPropagation) {
        ...
        if constexpr (UseParallelCarry) {
            ...
            // First Pass: Process convolution results to compute initial digits and local carries
            CarryPropagation<SharkFloatParams>((uint64_t*)shared_data,
                                               grid,
                                               block,
                                               thread_start_idx,
                                               thread_end_idx,
            ...
            SerialCarryPropagation<SharkFloatParams>((uint64_t*)shared_data,
                                                     grid,
                                                     block,
                                                     thread_start_idx,
                                                     thread_end_idx,
    ...
    if constexpr (!SharkFloatParams::DisableFinalConstruction) {
        ...
        auto ExponentAndSign = [](cg::thread_block& block,
        ...
        ExponentAndSign(block, A, A, true, OutXX, shift_digits_xx, X2_AdditionalFactorsOfTwo);

        constexpr auto XY_AdditionalFactorsOfTwo = 1;
        ExponentAndSign(block, A, B, false, OutXY, shift_digits_xy, XY_AdditionalFactorsOfTwo);

        constexpr auto Y2_AdditionalFactorsOfTwo = 0;
        ExponentAndSign(block, B, B, true, OutYY, shift_digits_yy, Y2_AdditionalFactorsOfTwo);

        auto Finalize = [](cg::grid_group& grid,
                           cg::thread_block& block,
        ...
        Finalize(grid, block, resultEntriesXX, highest_nonzero_index_xx, shift_digits_xx, OutXX);

        Finalize(grid, block, resultEntriesXY, highest_nonzero_index_xy, shift_digits_xy, OutXY);

        Finalize(grid, block, resultEntriesYY, highest_nonzero_index_yy, shift_digits_yy, OutYY);
    }
}
