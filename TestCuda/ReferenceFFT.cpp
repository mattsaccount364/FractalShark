...
add64carry(uint64_t a, uint64_t b, uint64_t& carry)
{
    uint64_t s = a + b;
    uint64_t c = (s < a);
...
mont_mul(uint64_t a, uint64_t b)
{
    ...
    uint64_t m = t_lo * GoldilocksNTT::NINV;

    // m*P (128-bit)
    uint64_t mp_lo, mp_hi;
    mul64wide(m, GoldilocksNTT::P, mp_lo, mp_hi);
    ...
    // r = u / 2^64; ensure r < P
    uint64_t r = u_hi;
    if (carry1 || r >= GoldilocksNTT::P) // <-- include the high-limb carry-out
        r -= GoldilocksNTT::P;
...
static void
bit_reverse_inplace_u64(uint64_t* A, uint32_t N, uint32_t stages)
{
    for (uint32_t i = 0; i < N; ++i) {
        uint32_t j = reverse_bits32(i, stages) & (N - 1);
        if (j > i)
            std::swap(A[i], A[j]);
    }
}
...
static PlanPrime
build_plan_prime(int n32, int b_hint = 26, int margin = 2)
{
    using namespace GoldilocksNTT;
    PlanPrime pl{};
    pl.ok = false;
    ...
    if (b > 30)
        b = 30;
    uint32_t L = ceil_div_u32((uint32_t)totalBits, (uint32_t)b);
    uint32_t N = next_pow2_u32(2u * L);
    int lgN = ceil_log2_u32(N);
    int bmax = (64 - margin - lgN) / 2;
    if (b > bmax)
        b = bmax;
    if (b < 16)
        b = 16;
    L = ceil_div_u32((uint32_t)totalBits, (uint32_t)b);
    ...
    if ((PHI % (2ull * (uint64_t)N)) != 0ull) {
        ...
        for (int b_try = b + 1; b_try <= b_up_max; ++b_try) {
            uint32_t L2 = ceil_div_u32((uint32_t)totalBits, (uint32_t)b_try);
            uint32_t N2 = next_pow2_u32(2u * L2);
            if ((PHI % (2ull * (uint64_t)N2)) == 0ull) {
                b = b_try;
                L = L2;
                N = N2;
                lgN = ceil_log2_u32(N2);
                fixed = true;
                break;
            }
        }
...
template <class P>
static uint64_t
read_bits_simple(const HpSharkFloat<P>* X, int64_t q, int b)
{
...
static void
unpack_prime_to_final128(const uint64_t* A_norm, const PlanPrime& pl, uint64_t* Final128, size_t Ddigits)
{
    using namespace GoldilocksNTT;
    std::memset(Final128, 0, sizeof(uint64_t) * 2 * Ddigits);
                   ...
                   DebugHostCombo<SharkFloatParams>& debugCombo)
{
    ...
    auto run_conv = [&](const HpSharkFloat<P>* XA,
        ...
        unpack_prime_to_final128(X_norm.data(), pl, Final128.data(), Ddigits);
        OUT->SetNegative(isNegative);
        NormalizeFromFinal128LikeKaratsuba<P>(*OUT, inA, inB, Final128.data(), Ddigits, addFactorOfTwo);
    };
