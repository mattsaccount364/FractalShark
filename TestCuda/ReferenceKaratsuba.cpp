...
// Add partial sums (Arr) into Dest at given 64-bit word offset, with carry propagation.
static void AddPartialSum(std::vector<uint64_t> &Dest, const std::vector<uint64_t> &Arr, int offset) {
    int len = (int)Arr.size();
    uint64_t carry = 0;
    for (int i = 0; i < len; i++) {
        uint64_t sum;
        uint64_t A_ = Dest[i + offset];
        uint64_t B_ = Arr[i];
        add64withCarry(A_, B_, carry, sum, carry);
        Dest[i + offset] = sum;
    }
    // If carry remains, propagate further
    int idx2 = offset + len;
    while (carry > 0 && idx2 < (int)Dest.size()) {
        uint64_t sum;
        add64withCarry(Dest[idx2], 0ULL, carry, sum, carry);
        Dest[idx2] = sum;
        idx2++;
    }
}

// add128, subtract128 as before:
static void add128(
    ...
    uint64_t carry = 0;
    add64withCarry(a_low, b_low, 0, result_low, carry);
    uint64_t sum;
    add64withCarry(a_high, b_high, carry, sum, carry);
    result_high = sum;
    // carry should not overflow here for practical cases.
}
...
static void subtract128(
    ...
    uint64_t borrow = 0;
    // Sub low
    uint64_t diff_low = a_low - b_low;
    borrow = (a_low < b_low) ? 1ULL : 0ULL;

    // Sub high with borrow
    uint64_t diff_high = a_high - b_high - borrow;
    // If we assume a>=b, no final borrow remains.

    result_low = diff_low;
    result_high = diff_high;
}
...
void MultiplyHelperKaratsubaV2(
    ...
    start with this implementation and see how we can fix it.
    ...
    // Compare and subtract arrays as before
    ...
    auto SubtractArrays = [&](const uint32_t *A_, int A_len, const uint32_t *B_, int B_len, uint32_t *Res) {
        ...
        return (uint32_t)borrow; // assuming A>=B no final borrow
        };
    ...
    // Always produce exactly N digits:
    if (significant_digits < N) {
        // Fewer than N digits => pad with leading zeros (on the left)
        // We'll still place the MSB at Out->Digits[0], so we just note that we have fewer significant digits
        // but still produce N. The extra digits at the top will be zeros.
        significant_digits = N;
    } else if (significant_digits > N) {
        // More than N digits => discard extra top digits so that we produce exactly N digits.
        // This means we take the top N digits from tempDigits (the last N of them are the most significant N)
        // effectively we will just ignore the extra since we are only copying N digits anyway.
        significant_digits = N;
    }

    // Set exponent without shifts for now:
    Out->Exponent = A->Exponent + B->Exponent;

    // Now copy out exactly N digits in reverse order:
    for (int i = 0; i < N; i++) {
        // i=0 => MSB = tempDigits[significant_digits-1]
        // i=1 => next = tempDigits[significant_digits-2]
        // ...
        // i=N-1 => LSB = tempDigits[significant_digits-N]
        int src_idx = significant_digits - 1 - i;
        uint32_t val = (src_idx >= 0 && src_idx < (int)tempDigits.size()) ? tempDigits[src_idx] : 0;
        Out->Digits[i] = val;
    }
