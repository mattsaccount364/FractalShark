...
void TestTernaryOperatorTwoNumbersRawNoSignChange (
    ...
    if constexpr (SharkFloatParams::HostVerbose) {
        ...
        std::cout << "Z: " << zNum.ToString() << std::endl;
        std::cout << "Z hex: " << zNum.ToHexString() << std::endl;
    }
    ...
    auto TestHostKaratsuba = [&](
        ...
        res &= CheckAgainstHost(testNum, "CustomHighPrecisionV2XY", mpfHostResultXY1, hostKaratsubaOutXYV2);
    ...
    auto TestHostAdd = [&](
        ...
        std::vector<DebugStateHost<SharkFloatParams>> &debugStates) -> bool {
            ...
            bool res = true;
            res &= CheckAgainstHost(testNum, "CustomHighPrecisionV2XY1", mpfHostResultXY1, hostAddResult1);
            res &= CheckAgainstHost(testNum, "CustomHighPrecisionV2XY2", mpfHostResultXY2, hostAddResult2);
    ...
    if constexpr (IncludeSigns) {
...

        {
            ...
            TestTernaryOperatorTwoNumbersRawNoSignChange<SharkFloatParams, sharkOperator>(
                testNum, xNumCopy, yNumCopy, zNumCopy, mpfXCopy, mpfYCopy, mpfZCopy);
            testNum++;
        }
...

        {
            resetCopy();
            if constexpr (!SharkTestForceSameSign) {
                negateMpfAndHp(mpfZCopy, zNumCopy);
            }
            printTest(testNum);
            TestTernaryOperatorTwoNumbersRawNoSignChange<SharkFloatParams, sharkOperator>(
                testNum, xNumCopy, yNumCopy, zNumCopy, mpfXCopy, mpfYCopy, mpfZCopy);
...
void TestTernaryOperatorTwoNumbers (
    ...
    auto curTest = [&]() {
        ...
        if constexpr (SharkFloatParams::HostVerbose) {
            ...
            std::cout << "MpfZ: " << MpfToString<SharkFloatParams>(mpfZCopy, HpSharkFloat<SharkFloatParams>::DefaultPrecBits) << std::endl;
        ...
        std::unique_ptr<HpSharkFloat<SharkFloatParams>> zNum = std::make_unique<HpSharkFloat<SharkFloatParams>>();
        ...
        MpfToHpGpu(mpfZCopy, *zNum, HpSharkFloat<SharkFloatParams>::DefaultPrecBits);

        TestTernaryOperatorTwoNumbersRaw<SharkFloatParams, sharkOperator, false>(
            testNum, *xNum, *yNum, *zNum, mpfXCopy, mpfYCopy, mpfZCopy);
...
void TestTernarySpecial (
    int testNum,
    std::vector<uint32_t> &digits1,
    std::vector<uint32_t> &digits2,
    std::vector<uint32_t> &digits3)
{
    mpf_t x, y, z;
    mpf_init(x);
    mpf_init(y);
    ...
    auto strLargeZ = Uint32ToMpf<SharkFloatParams>(digits3.data(), SharkFloatParams::HalfLimbsRoundedUp, z);
...
void TestTernarySpecialHelper(
    ...
    std::vector<uint32_t> testData3Copy;
    testData3Copy = testData3;
    testData3Copy.resize(SharkFloatParams::GlobalNumUint32);
    ...
    std::unique_ptr<HpSharkFloat<SharkFloatParams>> zNum{ std::make_unique<HpSharkFloat<SharkFloatParams>>(testData3Copy.data(), 0, false) };
...
bool TestAllBinaryOp(int testBase) {
    ...
    if constexpr (includeSet1) {
        ...
        TestTernaryOperatorTwoNumbers<SharkFloatParams, sharkOperator>(set + 10, "1", "2", "3");
